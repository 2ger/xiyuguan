    ...
    public function doMobileindex() {
      ...
      //var_dump($_W['member']);
        ...
        if ($reply['votelimit'] == 1) {
            ...
            $limits = "期限: " . date('Y-m-d H:i', $reply['starttime']) . " 至 " . date('Y-m-d H:i', $endtime);
        }
        $selects = "";
        if ($reply['votetype'] == 0) {
            $selects = "最多选择一项";
        ...
        $sharedesc = empty($reply['share_desc']) ? '亲，欢迎参加活动！' : $reply['share_desc'];
    ...
    public function doWebManage() {
        ...
        if (!empty($list)) {
            foreach ($list as &$item) {
                ...
                if ($vote['votelimit'] == 1) {
                    $limits = "允许 " . $vote['votetotal'] . " 人";
                } else {
                    $limits = "期限: " . date('Y-m-d H:i', $vote['starttime']) . " 至 " . date('Y-m-d H:i', $endtime);
                }
                $item['limits'] = $limits;
