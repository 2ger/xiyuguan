...
class J_moneyModuleSite extends WeModuleSite {
	public function doMobileAjax() {
		...
		if($operation=="login"){
            ...
            //称鱼，及收银员 进入收银界面
            if($item['pcate']==1 or $item['pcate']==9){
		...
		if($operation=="song"){
			...
			$price=$_GPC['price'];
            // 修改菜的type=1
			 pdo_update("weisrc_dish_order_goods",array("type"=>1),array("orderid"=>$orderid,"goodsid"=>$goodsid));
            // 订单表 金额 
            // $orderprice =pdo_fetch("select totalprice from ims_weisrc_dish_order where id=".$orderid);
            // $price = $orderprice['totalprice']-$price;
            // if($price<0){
		...
		if($operation=="ispay"){
			...
			pdo_update("weisrc_dish_order_goods",array("status"=>2),array("orderid"=>$orderid));
			pdo_update("weisrc_dish_order",array("status"=>2,"zhekou"=>$zhekou,"ispay"=>1),array("id"=>$orderid));
			$result=array(
				"success"=>1,
				"out_trade_no"=>$outTradeNo,
				"result_code"=>'SUCCESS',
		...
		if($operation=="getcounternoprintrecord"){
			...
			$list=pdo_fetchall("SELECT * FROM ".tablename('j_money_endday')." WHERE type=1 order by id desc");//交班信息
			$i=0;
			$templist=array();
			foreach($list as $row){
				if($i<10){
	...
	*/
	...
	public function doMobileIndex() {
		...
		$tablesid=$_GPC['tablesid'];
		$url='/index.php?i=1&c=entry&op=in&do=index&m=j_money&tablesid='.$tablesid;
		$ismobile=isMobile();
		...
		$end = time();
		//微信支付
		$weixin=pdo_fetch("SELECT SUM(cash_fee) as weixin FROM ".tablename('j_money_trade')." WHERE paytype=0 and createtime between $start and $end");
		$weixin=$weixin['weixin']/100;
		//支付宝支付
		$zfb=pdo_fetch("SELECT SUM(cash_fee) as zfb FROM ".tablename('j_money_trade')." WHERE paytype=1 and createtime between $start and $end");
		$zfb=$zfb['zfb']/100;
		//美团支付
		$meituan=pdo_fetch("SELECT SUM(cash_fee) as meituan FROM ".tablename('j_money_trade')." WHERE paytype=2 and createtime between $start and $end");
		$meituan=$meituan['meituan']/100;
		//现金支付
		$xianjin=pdo_fetch("SELECT SUM(cash_fee) as xianjin FROM ".tablename('j_money_trade')." WHERE paytype=3 and createtime between $start and $end");
		$xianjin=$xianjin['xianjin']/100;
		//刷卡支付
		$shuaka=pdo_fetch("SELECT SUM(cash_fee) as shuaka FROM ".tablename('j_money_trade')." WHERE paytype=4 and createtime between $start and $end");
		$shuaka=$shuaka['shuaka']/100;
		//今日营业额
		$today=pdo_fetch("SELECT SUM(cash_fee) as todaymoney FROM ".tablename('j_money_trade')." WHERE createtime between $start and $end");
		$today=$today['todaymoney']/100;
    ...
    // public $table_tables_order = "weisrc_dish_tables_order";
        ...
        if ($operation == 'changetable') {
            ...
            pdo_fetch("UPDATE ims_weisrc_dish_tables set status=1 WHERE id=".$tablesnet);
            //把订单转过来
            $orderid = pdo_fetch('SELECT id from ims_weisrc_dish_order WHERE ispay = 0 and tables ='.$tablesid.' order by id desc limit 1');
            $isok = pdo_fetch("UPDATE ims_weisrc_dish_order set tables=".$tablesnet." WHERE id=".$orderid['id']);
//把菜单转过来
            pdo_fetch("UPDATE ims_weisrc_dish_order_goods set tablesid=".$tablesnet." WHERE orderid=".$orderid['id']);
            // message('操作成功！', $this->createWebUrl('index', array('op' => 'in', 'tablesid' => $change, 'i' => 1)), 'success');
            $url ="index.php?i=1&c=entry&op=in&do=index&m=j_money&tablesid=".$tablesnet;
            message("操作成功了！",$url,'success');
        }
        // 转台功能 end 
		...
		if($operation=='display'){
            ...
            //满减活动列表
            $manjian=pdo_fetchall("SELECT * FROM ".tablename('j_money_manjian')." where status=1 order by manjian");
            ...
            if(!empty($tablesid)){
            	...
            	if(isset($_GPC['fishclik'])){
				 ...
				 //    $data=array(
					...
					// 'taste'=>$flavor,
				 //    );
				$fistorder=pdo_fetch("SELECT * FROM ims_weisrc_dish_order WHERE status<=1 and ispay=0 and tables=".$tablesid);
				if($fistorder['id']){
  
                    // var_dump($fistorder);
                    // die();
				    $select=pdo_fetch("SELECT * FROM ims_weisrc_dish_order_goods WHERE goodsid=6 AND status<=1 AND tablesid=".$tablesid);//查找鱼
				    $firstprice=$select['price']*$select['total'];//原鱼的总价
				    if(!empty($select['id'])){
				        // $select2=pdo_fetch("SELECT * FROM ims_weisrc_dish_order WHERE id=".$select['orderid']);
				    //查找上级
                        $selectfrom = pdo_fetch("SELECT agentid from ims_ewei_shop_member where openid='".$fistorder['from_user']."'");
                   //查找上级id 
                        if($selectfrom['agentid']!=0){
                            $selectfrom2 = pdo_fetch("SELECT uid from ims_ewei_shop_member where id=".$selectfrom['agentid']);
                            $selectfrom3 = pdo_fetch("SELECT credit2 from ims_mc_members where uid=".$selectfrom2['uid']);
                            $lirunprice=$fistorder['totalprice']*0.03;
                            $lirunprice2=$selectfrom3['credit2']-$lirunprice;
                        }
                  
				        $secondprice=$fistorder['totalprice']-$firstprice;
				        $endprice=$secondprice+$price;
				    
				        $update=pdo_fetch("UPDATE ims_weisrc_dish_order set totalprice=".$endprice." WHERE id=".$select['orderid']);
				        $update2=pdo_fetch("UPDATE ims_weisrc_dish_order_goods set price=".$oneprice.",total=".$weight.",taste='".$flavor."',step1='".time()."' WHERE goodsid=6 AND status<=1 AND tablesid=".$tablesid);
				        if($selectfrom2['uid']){
				    	    $select3=pdo_fetch("SELECT totalprice FROM ims_weisrc_dish_order WHERE id=".$select['orderid']);
				    	    $agprice=$select3['totalprice']*0.03;
				    	    $aglirun=$lirunprice2+$agprice;
                            $update3 =pdo_fetch("UPDATE ims_mc_members SET credit2=".$aglirun." WHERE uid=".$selectfrom2['uid']);
                        }
                        //修改鱼的重量
                        // $tableUrl ="app/index.php?i=1&c=entry&op=in&do=index&m=j_money&tablesid=".$tablesid; 
				        if($update2=="")message("操作成功 ！",$tableUrl,'success');
				    }else{
				    	//订单中无鱼 插入鱼
				    	$allprice=$price+$fistorder['totalprice'];
				    	$totalnum=$fistorder['totalnum']+1;
				    	$insert=pdo_fetch("INSERT INTO ims_weisrc_dish_order_goods(weid,storeid,orderid,goodsid,price,total,dateline,tablesid,taste) VALUES({$_W['uniacid']},1,{$fistorder['id']},6,{$oneprice},{$weight},{$date},{$tablesid},'{$flavor}')");
				    	$update=pdo_fetch("UPDATE ims_weisrc_dish_order SET totalnum=".$totalnum.", totalprice=".$allprice);
				       //pdo_insert("ims_weisrc_dish_cart",$data);
				        if($insert=="")message("操作成功了！",$url,'success');
				    }
				}else{
                        // 未下单时， 插入购物车
				    	$insert=pdo_fetch("INSERT INTO ims_weisrc_dish_cart(weid,storeid,goodsid,goodstype,price,total,tablesid,taste) VALUES({$_W['uniacid']},1,6,1,{$oneprice},{$weight},{$tablesid},'{$flavor}')");
                        // var_dump('dddd');
                        // die();
                        pdo_update("weisrc_dish_tables",array('status'=>1),array("id"=>$tablesid));
				        if($insert=="")message("操作成功了！",$url,'success');
				}
                ...
                if(isset($_GPC['specialclik'])){
				    ...
				    $select=pdo_fetch("SELECT * FROM ims_weisrc_dish_order WHERE status<=1 and ispay=0 AND tables=".$tablesid);
				    if($select['id']){
				    	$allprice=$price1+$select['totalprice'];
				    	$totalnum=$select['totalnum']+$weight1;
				    	$insert=pdo_fetch("INSERT INTO ims_weisrc_dish_order_goods(weid,storeid,orderid,goodsid,price,total,dateline,tablesid,taste) VALUES({$_W['uniacid']},1,{$select['id']},130,{$oneprice1},{$weight1},{$date},{$tablesid},'{$flavor1}')");
				    	$update=pdo_fetch("UPDATE ims_weisrc_dish_order SET totalnum=".$totalnum.", totalprice=".$allprice);
				    	if($update=="")message("操作成功了！",$url,'success');
				    }else{
				    	$insert=pdo_fetch("INSERT INTO ims_weisrc_dish_cart(weid,storeid,goodsid,goodstype,price,total,tablesid,taste) VALUES({$_W['uniacid']},1,130,9,{$oneprice1},{$weight1},{$tablesid},'{$flavor1}')");
				    	if($insert=="")message("操作成功了！",$url,'success');
				    }
               ...
               // 所有消费
                    $xiaofei=pdo_fetch("SELECT sum(total*price) as total from ".tablename('weisrc_dish_order_goods')." WHERE (status =0 or status =1) and type !=2 and orderid=".$orderid);//所有消费金额
                    //应收：非退，送菜总金额
                   ...
                   $kedazhe=$xiaofei['ys']-$buzhekou['price'];//可折扣价格
...
// var_dump($orderinfo);
$orderinfo['ys'] = $orderinfo['totalprice'] - $orderinfo['zhekou'];
        //桌号信息
        $tableinfo=pdo_fetch("SELECT t.title as id,z.title as zone from ".tablename('weisrc_dish_tables')." t left join ".tablename('weisrc_dish_tablezones')." z on t.tablezonesid = z.id WHERE t.id=".$tablesid);
        //菜单信息 
        $order_goods=pdo_fetchall("SELECT a.title,b.type,b.total,b.price,(b.total*b.price) as ttprice,b.taste as flavor from ".tablename('weisrc_dish_order_goods')." as b join ".tablename('weisrc_dish_goods')." as a on a.id=b.goodsid join ".tablename('weisrc_dish_order')." as o on o.id=b.orderid WHERE o.id=".$orderid);
        // $order_goods=pdo_fetchall("SELECT a.title,b.total,b.price,(b.total*b.price) as ttprice,b.taste as flavor from ".tablename('weisrc_dish_order_goods')." as b join ".tablename('weisrc_dish_goods')." as a on a.id=b.goodsid WHERE b.status =0 and b.tablesid=".$tablesid);
        // 消费金额
                    $xiaofei=pdo_fetch("SELECT sum(total*price) as total from ".tablename('weisrc_dish_order_goods')." WHERE (status =0 or status =1) and type !=2 and orderid=".$orderid);//所有消费金额
                    //应收：非退，送菜总金额
                    $ys=pdo_fetch("SELECT sum(total*price) as total from ".tablename('weisrc_dish_order_goods')." WHERE (status =0 or status =1) and type =0 and orderid=".$orderid);//所有消费金额
...
// 确认订单
    ...
    public function doMobileQueueorder2() {
        ...
        if ($operation == 'detail') {
            ...
            if (checksubmit('submit')) {
                ...
                $url='index.php?i=1&c=entry&op=detail&storeid='.$storeid.'&queueid='.$queueid.'&do=queueorder2&m=j_money';
     ...
     public function doMobileQueuesetting2()
    {
        ...
        if ($operation == 'post') {
            ...
            if (checksubmit('submit')) {
                ...
                $url='index.php?i=1&c=entry&op=display&storeid='.$storeid.'&do=queuesetting2&m=j_money';
                message('操作成功！', $url,'success');
            }
        } else if ($operation == 'display') {
            ...
            if (checksubmit('submit')) {
                ...
                $url='index.php?i=1&c=entry&op=setting&storeid='.$storeid.'&do=queuesetting2&m=j_money';
    ...
    public function doMobileReservation()
    {
        ...
        if ($operation == 'post') {
            ...
            $url='index.php?i=1&c=entry&op=display&storeid='.$storeid.'&do=reservation&m=j_money';
                message('操作成功！', $url,'success');
	...
	public function doMobileCounthistory() {
		...
		if($_GPC['start'] && $_GPC['end'])$where.=" and createtime between '".$starts."' and '".$end."'";
		...
		if(isset($_GPC['jiaoban'])){
			$suser=pdo_fetch("SELECT * FROM ".tablename('j_money_endday')." where type=1 order by id DESC");
			if(empty($suser)){
			...
			$sprice=pdo_fetch("SELECT SUM(total_fee) as total,SUM(cash_fee) as cash from ".tablename('j_money_trade')." where status=1 and createtime>=".$stime." and createtime<=".$etime." and userid=".$_GPC['islogin']);
			$sprice['cash']=$sprice['cash']/100;
			$sprice['total']=$sprice['total']/100;

			$insert=pdo_fetch("INSERT INTO ims_j_money_endday(userid,starttime,endtime,revenue,totalprice,createtime,type) values(".$_GPC['islogin'].",".$stime.",".$etime.",".$sprice['cash'].",".$sprice['total'].",".time().",1)");
			...
			if($row['status']==1){
				if($row['total_fee']){
					if($row['paytype']==0){
						...
						$payAry['meituan']['all']=$payAry['meituan']['all']+$row['total_fee'];
						$payAry['meituan']['all-count']=$payAry['meituan']['all-count']+1;
					}elseif($row['paytype']==3){
						$payAry['xianjin']['all']=$payAry['xianjin']['all']+$row['total_fee'];
						$payAry['xianjin']['all-count']=$payAry['xianjin']['all-count']+1;
					}elseif($row['paytype']==4){
						$payAry['shuaka']['all']=$payAry['shuaka']['all']+$row['total_fee'];
...
//----
	...
	*/
	public function doWebMember() {
		...
		if($operation=="display"){
			$list=pdo_fetchall("SELECT a.*,b.companyname,s.title FROM ".tablename('j_money_user')." a left join ".tablename('j_money_group')." b on a.pcate = b.id left join ".tablename('weisrc_dish_stores')." s on s.id=a.storeid WHERE a.weid = '{$_W['uniacid']}' order by a.pcate desc");
			//添加员工考勤2016.10.25
			foreach ($list as $key => $value) {
				//上月
