...
class weisrc_dishModuleSite extends WeModuleSite
{
    ...
    public $member_code = '111111111111111';
    public $feyin_key = '111111111111111';
    public $device_no = '111111111111111';
    ...
    public $_appid = '111111111111111';
    public $_appsecret = '111111111111111';
    public $_accountlevel = '111111111111111';
    public $_account = '111111111111111';

    public $_weid = '111111111111111';
    public $_fromuser = '';//$_COOKIE['username'];//取cookie  原 ''
    public $_nickname = '';//$_COOKIE['username'];
    public $_headimgurl = '111111111111111';

    public $_auth2_openid = '111111111111111';
    public $_auth2_nickname = '111111111111111';
    public $_auth2_headimgurl = '111111111111111';
    public $_lat = '';
    public $_lng = '';
    ...
    public $table_materialzones = "weisrc_dish_materialzones";
    public $table_materials = "weisrc_dish_materials";
    //table_materialzones
    ...
    public function doMobileWapList()
    {
        ...
        //判断是否关注公众号17/1/17
        $followed = pdo_fetch('select follow from ' . tablename('mc_mapping_fans') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $from_user, ':uniacid' => $weid));
        // var_dump($followed);
        if(empty($followed['follow'])){
         //   $url = 'https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=MzAxMDkzODI0MQ==&scene=110#wechat_redirect';
        //     header("location:$url");
        }
        //判断是否关注公众号17/1/17end
        ...
        if ($mode == 1) {
            //本人订单
            $select=pdo_fetch("SELECT * FROM ims_weisrc_dish_order WHERE (status=0 or status =1) and ispay=0 AND tables=".$tablesid." and from_user = '".$from_user."' order by id desc");
            //非本人订单
            $select2=pdo_fetch("SELECT * FROM ims_weisrc_dish_order WHERE (status=0 or status =1) and ispay=0 AND tables=".$tablesid." and from_user != '".$from_user."' order by id desc");

            // 以上为用，查看有未完成订单
            $select3=pdo_fetch("SELECT * FROM ims_weisrc_dish_order WHERE (status=0 or status =1) and ispay=0 AND tables=".$tablesid." order by id desc");
            if($select){
                $add =1;//用户加菜
                $tips ="用户加菜";
            }
            if($select3){// 不允许用户自行加菜
                // $tips ="本桌有未付款订单，请联系服务员点菜！";
                // $undone =1;//有未完成订单
            }
        ...
        if(!empty($logid)){
          ...
          $opp = $_GPC['opp'];
            $url = 'index.php?i='.$_W['uniacid'].'&c=entry&do=opera&m=weisrc_dish&tablesid='.$tablesid.'&opp='.$opp;
            header("location:$url");
        ...
        $children = array();
        //不显示分类
        $category = pdo_fetchall("SELECT * FROM " . tablename($this->table_category) . " WHERE weid = :weid AND storeid=:storeid  and name not in ('特殊类','活杀黑鱼') ORDER BY  displayorder DESC,id ", array(':weid' => $weid, ':storeid' => $storeid));
        
//是否称鱼 
        $cartfish = pdo_fetch("SELECT count(*) as total FROM " . tablename($this->table_cart) . " WHERE  tablesid=:tablesid  AND weid=:weid", array(':tablesid' => $tablesid, ':weid' => $weid));
      $fish =$cartfish['total'];
       // var_dump($cartfish['total']);
       // die();
        // $cid = intval($category[0]['id']);
      $cate = $_GPC['categoryid'];
      if(empty($cate)){
        $cid = intval($category[0]['id']);
        $totalcount = 0;
        $totalprice = 0;
        ...
        if ($mode == 1) {
            ...
            $jump_url2 = $this->createMobileurl('wapmenu', array('from_user' => $from_user, 'storeid' => $storeid, 'mode' => $mode, 'tablesid' => $tablesid, 'add' => 1), true);//加菜url
        } elseif ($mode == 2) {
    ...
    //     var_dump($from_user);
     ...
     //   $from_user = 'testuser';
        ...
        $cart = pdo_fetchall("SELECT *,a.taste as flavor FROM " . tablename($this->table_cart) . " a LEFT JOIN " . tablename('weisrc_dish_goods') . " b ON a.goodsid=b.id WHERE a.weid=:weid AND (a.from_user=:from_user or a.tablesid=:tables) AND a.storeid=:storeid order by a.id DESC", array(':weid' => $weid, ':from_user' => $from_user, ':storeid' => $storeid, ':tables' => $tables));//修改购物车中所有鱼可下单 group by a.goodsid
        $totalcount = 0;
        $totalprice = 0;
        foreach ($cart as $key => $value) {
            if($value['tasteid'] != ''){
             $cart[$key]['tasteid'] = pdo_fetchall("select * from ims_weisrc_dish_goods_taste WHERE id in (".$value['tasteid'].")");
            }
            $totalcount = $totalcount + $value['total'];
            $totalprice = $totalprice + $value['total'] * $value['price'];
        }
    ...
    public function doMobileCancelOrder()
    {
        ...
        if ($id == 0) { //未选队列
            ...
            $update= pdo_fetch("update " . tablename($this->table_order_goods) . " set status=-1 where orderid=".$id);
        }
        $this->showMsg('操作成功!', 1);
    }
...
// var_dump($from_user);
 ...
 status=1 ORDER BY id DESC ", array(':weid' => $this->_weid, ':from_user' => $from_user));
        ...
        //已确认
        $order_list = pdo_fetchall("SELECT a.* FROM " . tablename($this->table_order) . " AS a LEFT JOIN " . tablename($this->table_stores) . " AS b ON a.storeid=b.id  WHERE a.status!=5 and a.from_user='{$from_user}' ORDER BY a.id DESC LIMIT 20");
       // var_dump($order_list);
        //die();
        //数量
...
//        $from_user = 'testuser';
        ...
        $order_goods = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_order_goods) . " WHERE orderid=".$id);
        if(!$order_goods){//说明是加菜的订单, 显示源订单
            $orderinfo = pdo_fetch("SELECT addto FROM " . tablename($this->table_order) . " WHERE id=".$id);
            $id = $orderinfo['addto'];
        }
...
//判断是否关注公众号
        ...
        // var_dump($followed);
        if(empty($followed['follow'])){
            $url = 'http://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=MzAxMzczMjY1MA==&scene=123#wechat_redirect';
             // header('HTTP/1.1 301 Moved Permanently');
  // header('Location: ' .$url);
            // exit();
                 // header("location:$url");
            // echo "<cenger><h1><a href=".$url.">关注我们</a></h1></cenger>";
            // die();
            message('点菜成功！关注我们，查看上菜进度', $url);
        }else{
            // 已关注
            // $url2 = 'http://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=MzAxMzczMjY1MA==&scene=110#wechat_redirect';
            // message('关注我们，查看上菜进度'.$url2, $url2);
        }

        if (empty($order)) {
         //   var_dump($order);
            message('加菜订单已合并，请查看首次订单！');
        }

        if(!empty($agid)){
        	$agprice=$order['totalprice']*0.03;//返上级利润

        //查找上级
        $selectfrom = pdo_fetch("SELECT agentid from ims_ewei_shop_member where openid='".$from_user."'");
        //查找上级id
        if(!empty($selectfrom['agentid'])){
        $selectfrom2 = pdo_fetch("SELECT uid from ims_ewei_shop_member where id=".$selectfrom['agentid']);
        if(!empty($selectfrom2['uid'])){
        $update =pdo_fetch("UPDATE ims_mc_members SET credit2=credit2+".$agprice." WHERE uid=".$selectfrom2['uid']);
        }
        
        //pdo_update('mc_memners', array('credit2' => $agprice), array('uid' => $selectfrom2['uid']));
        }
        ...
        $order['goods'] = pdo_fetchall("SELECT a.*,b.*,a.id as orderGoodsId,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id WHERE a.weid = '{$weid}' and a.orderid={$order['id']}");

        $orders = pdo_fetch("SELECT * FROM " . tablename($this->table_order_goods) . " WHERE weid = '{$weid}' and orderid={$order['id']} and goodsid=129"); 


        if (checksubmit()) {
            $zhaocha = $_GPC['zhaocha'];
            ...
            $sqlorder = pdo_fetch("SELECT a.* FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id WHERE a.weid = '{$weid}' and b.pcate=1 and a.orderid={$order['id']}");
            $url2 = "index.php?'c=entry&do=orderdetail&m=weisrc_dish&orderid=".$sqlorder['orderid'];
            
            if (isset($_GPC['submit1']) && $pingjia!=0 && $sqlorder['id']!="") {
                    $pingjias=pdo_fetch("UPDATE ims_weisrc_dish_order_goods SET pingjia=". $pingjia ." where id=".$sqlorder['id']);
                    if($pingjias==false){
                        message('评价成功！', $url2, 'success');
                    }else{
                        echo "<script language='javascript'>alert('评价失败!');history.back();</script>";
                    }
            }elseif(isset($_GPC['submit1']) && $pingjia==0 && $sqlorder['id']!=""){
                    echo "<script language='javascript'>alert('请选择评价内容!');history.back();</script>";
            }elseif(isset($_GPC['submit1']) && $sqlorder['id']==""){
                echo "<script language='javascript'>alert('该订单不参与评价!');history.back();</script>";
            }
            //找茬
            if($sqlorder['zhaocha']==""){
                $totalprice=$order['totalprice']-$newNum;
                if (isset($_GPC['submit2']) && $zhaocha!=0 && $sqlorder['id']!="") {
                    $zhaochas=pdo_fetch("UPDATE ims_weisrc_dish_order_goods SET zhaocha=". $zhaocha .",beizhu='". $comment."' where id=".$sqlorder['id']);
                    $zhaochas1=pdo_fetch("INSERT INTO ims_weisrc_dish_order_goods(weid,storeid,orderid,goodsid,dateline,stepnow,price,total,tablesid,taste,status) VALUES(".$weid.",1,".$sqlorder['orderid'].",131,".time().",5,".$newNums.",1,".$sqlorder['tablesid'].",'找茬',1)");

                    $totalmoney=pdo_fetch("UPDATE ims_weisrc_dish_order SET totalprice=". $totalprice ." where id=".$sqlorder['orderid']);
                    if($zhaochas==false){
                        message('找茬成功！本单可减免'.$newNum.'元', $url2, 'success');
                    }else{
                        echo "<script language='javascript'>alert('找茬失败!');history.back();</script>";
                    }
                }elseif(isset($_GPC['submit2']) && $zhaocha==0 && $sqlorder['id']!=""){
                    echo "<script language='javascript'>alert('请选择找茬内容!');history.back();</script>";
                }elseif(isset($_GPC['submit2']) && $sqlorder['id']==""){  
                 echo "<script language='javascript'>alert('该订单不支持找茬!');history.back();</script>";
                }
                // 
            }else{
                echo "<script language='javascript'>alert('您已找茬，不能进行重复操作!');history.back();</script>";
            }
    ...
    //打赏列表
        public function doMobileEvaluatelist()
    {
        ...
        //操作人员
        $people=pdo_fetch("SELECT * FROM " . tablename($this->table_order_goods) . " WHERE goodsid=6 and orderid=".$orderid);
        if(isset($_GPC['submit'])){
            $man=$_GPC['people'];
            $dashang=$_GPC['money'];
            $sql=pdo_fetch("INSERT INTO dashang(storeid,orderid,man,dashang,timeline) VALUES(1,". $orderid .",'". $man ."','". $dashang ."',".time().")");
                if($sql==false){
                   message('打赏成功！', $url, 'success');
                }else{
                   echo "<script language='javascript'>alert('打赏失败!');history.back();</script>";
            }

        }

        $list=pdo_fetchall("SELECT * FROM dashang WHERE orderid=".$orderid);

        include $this->template($this->cur_tpl . '/evaluatelist');
    }
    //打赏
    public function doMobileEvaluate()
    {
        ...
        $idd = $_GPC['id'];
        $orderid = $_GPC['orderid'];
        $url = 'index.php?i='.$_W['uniacid'].'&c=entry&do=orderdetail&m=weisrc_dish&orderid='.$orderid;
      ...
      if (checksubmit()) {
        if(isset($_GPC['money1'])){
            $sql=pdo_fetch("INSERT INTO dashang(storeid,tablesid,orderid,goodsid,man,dashang,timeline) VALUES(". $storeid .",". $tablesid .",". $orderid .",". $goodsid .",'". $man ."',2,".time().")");
                if($sql==false){
                   message('打赏成功！', $url, 'success');
                }else{
                   echo "<script language='javascript'>alert('打赏失败!');history.back();</script>";
            }
        }elseif(isset($_GPC['money2'])){
                $sql=pdo_fetch("INSERT INTO dashang(storeid,tablesid,orderid,goodsid,man,dashang,timeline) VALUES(". $storeid .",". $tablesid .",". $orderid .",". $goodsid .",'". $man ."',4,".time().")");
                if($sql==false){
                   message('打赏成功！', $url, 'success');
                }else{
                   echo "<script language='javascript'>alert('打赏失败!');history.back();</script>";
        ...
        include $this->template($this->cur_tpl . '/evaluate');
    }
...
//        $cur_nave = 'my';
 ...
 status=1 ORDER BY id DESC ", array(':weid' => $this->_weid, ':from_user' => $from_user));
        ...
        if (empty($storeids)) {
            //已确认
            $order_list = pdo_fetchall("SELECT a.* FROM " . tablename($this->table_order) . " AS a INNER JOIN " . tablename($this->table_stores) . " AS b ON a.storeid=b.id  WHERE a.status={$status} ORDER BY a.id DESC LIMIT 200");
            //数量
            $order_total = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_order) . " WHERE status=1 AND from_user='{$from_user}' ORDER BY id DESC");
        } else {
            //已确认
            $order_list = pdo_fetchall("SELECT a.* FROM " . tablename($this->table_order) . " AS a INNER JOIN " . tablename($this->table_stores) . " AS b ON a.storeid=b.id  WHERE a.status={$status} AND a.storeid in ('".$storeids."') ORDER BY a.id DESC LIMIT 200");
...
public function doMobileFish()
    {
        ...
        $opera['storeid'] = $_GPC['storeid'];
        $opera['tablesid'] = $_GPC['tablesid'];
        //$opera['step'] = $_GPC['step'];
       $logid = $_COOKIE['logid'];

if ($logid =='') {
    $urll = 'index.php?i='.$_W['uniacid'].'&c=entry&do=index&m=j_money';
   message('未登录', $urll, 'success');
}
//$_SESSION['yuangong'] ='';
$list = pdo_fetchall("SELECT a.*,b.thumb,b.title,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_stores) . " as c on a.storeid=c.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  WHERE a.weid = '{$weid}' and b.pcate = 1 and a.stepnow < 4 and a.status=1  order by a.reminder desc,a.dateline");  //and a.stepnow= {$opera['step']}  and a.stepnow >0
        $nowtime = time();
        foreach ($list as $key => $value) {
     ...
     //   var_dump($list);
        include $this->template($this->cur_tpl . '/fish');
    }
//腌鱼
public function doMobileYanyu()
    {
//         global $_W, $_GPC;
//         $weid = $this->_weid;
//         //$from_user = $_W['member']['mobile']; 
...
// if ($logid =='') {
//     $urll = 'http://shanghai.xyj0772.com/app/index.php?i='.$_W['uniacid'].'&c=entry&do=index&m=j_money';
//    message('未登录', $urll, 'success');
// }

// $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,c.title as sortname,d.title as tablesname FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_stores) . " as c on a.storeid=c.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  WHERE a.weid = '{$weid}' and b.pcate = 1 and a.stepnow = 0 and a.status=0");  //and a.stepnow= {$opera['step']}  and a.stepnow >0
//         $nowtime = time();
//         foreach ($list as $key => $value) {
//             $list[$key]['totalprice'] = $list[$key]['total']*$list[$key]['price'];
//             $cha = $nowtime - $list[$key]['dateline'];
...
//         }
     //   var_dump($list);
        include $this->template($this->cur_tpl . '/yanyu');
    }
//称鱼
public function doMobileChenyu()
   {
//         global $_W, $_GPC;
//         $weid = $this->_weid;
//         //$from_user = $_W['member']['mobile']; 
...
//         $opera['tablesid'] = $_GPC['tablesid'];
//         //$opera['step'] = $_GPC['step'];
//         $uid = $_SESSION['yuangong'];
// if ($uid =='') {
//    message('未登录', $this->createMobileUrl('login'), 'success');
// }

// $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,c.title as sortname,d.title as tablesname FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_stores) . " as c on a.storeid=c.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  WHERE a.weid = '{$weid}' and b.pcate = 1 and a.stepnow = 1 and a.status=0");  //and a.stepnow= {$opera['step']}  and a.stepnow >0
//        $nowtime = time();
//         foreach ($list as $key => $value) {
//             $list[$key]['totalprice'] = $list[$key]['total']*$list[$key]['price'];
//             $cha = $nowtime - $list[$key]['dateline'];
...
//         }
     //   var_dump($list);
        include $this->template($this->cur_tpl . '/chenyu');
    }
//杀鱼
public function doMobileShayu()
    {
//         global $_W, $_GPC;
//         $weid = $this->_weid;
//         //$from_user = $_W['member']['mobile']; 
...
// $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,c.title as sortname,d.title as tablesname FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_stores) . " as c on a.storeid=c.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  WHERE a.weid = '{$weid}' and b.pcate = 1 and a.stepnow = 2 and a.status=0");  //and a.stepnow= {$opera['step']}  and a.stepnow >0
//         $nowtime = time();
//         foreach ($list as $key => $value) {
//             $list[$key]['totalprice'] = $list[$key]['total']*$list[$key]['price'];
//             $cha = $nowtime - $list[$key]['dateline'];
...
//         }
     //   var_dump($list);
        include $this->template($this->cur_tpl . '/shayu');
    }
//煮鱼
public function doMobileZhuyu()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        ...
        if($revoke ==1){
            //单独列出鱼
            // $listFish =pdo_fetchall("SELECT a.*,b.title,c.title as sortname,d.title as tablesname from ims_weisrc_dish_fish a left join ims_weisrc_dish_tables b on a.tablesid = b.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  where a.storeid=".$storeid." and done=1 order by a.createtime");
            //去除鱼以外的
            $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation in (3,12) and a.status=1 and a.stepnow > 0 and a.storeid=".$storeid."  order by a.dateline desc limit 30");//and a.weid = '{$weid}' 
        }else{
            //单独列出鱼
            // $listFish =pdo_fetchall("SELECT a.*,b.title,c.title as sortname,d.title as tablesname from ims_weisrc_dish_fish a left join ims_weisrc_dish_tables b on a.tablesid = b.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  where a.storeid=".$storeid." and done in (0,6) order by a.createtime");
            //去除鱼以外的
            $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation in (3,12)  and a.status=1 and a.stepnow < 1 and a.storeid=".$storeid."  order by a.dateline");//and a.weid = '{$weid}'
            // $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE ((b.pcate = 1 and a.stepnow = 0) or (b.pcate = 6 and a.stepnow < 1) or (b.id = 130 and a.stepnow < 1)) and a.weid = '{$weid}' and a.status=1 and b.pcate !=1 and a.storeid=".$storeid."  order by a.dateline");  // and a.type <2 type =2 退菜 
            $nowtime = time();
            foreach ($list as $key => $value) {
                $list[$key]['totalprice'] = $list[$key]['total']*$list[$key]['price'];
             ...
             if ($cha >300 && $cha <$category && $value['reminder']==0) {
                $list[$key]['change'] = "f-blue";
            }elseif($value['reminder']==1 or $cha>$category){
                $list[$key]['change'] = "f-red";
            }

        }
      }
     //   var_dump($list);
        include $this->template($this->cur_tpl . '/zhuyu');
    }
    //牛肉间 切配间
    public function doMobileniurou()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        $storeid = $_SESSION['storeid'];
       ...
       // $from_user = $this->_fromuser;
        ...
        $revoke = $_GPC['revoke'];
        if($revoke ==1){
            $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation=15  and a.status=1 and a.stepnow > 0 and a.storeid=".$storeid."  order by a.dateline desc limit 30");//and a.weid = '{$weid}'
        }else{
        $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation=15  and a.stepnow < 1 and a.status=1 and a.storeid=".$storeid."  order by a.reminder desc,a.dateline");//and a.weid = '{$weid}'
        // var_dump($storeid);
        $nowtime = time();
        foreach ($list as $key => $value) {
            $list[$key]['totalprice'] = $list[$key]['total']*$list[$key]['price'];
        ...
        include $this->template($this->cur_tpl . '/niurou');
    }
//冷菜系
    public function doMobileLencai()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        $storeid = $_SESSION['storeid'];
       ...
       // $from_user = $this->_fromuser;
        ...
        $revoke = $_GPC['revoke'];
        if($revoke ==1){
            $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation=7  and a.status=1 and a.stepnow > 0 and a.storeid=".$storeid."  order by a.dateline desc limit 30");//and a.weid = '{$weid}'
        }else{
        $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation=7  and a.stepnow < 1 and a.status=1 and a.storeid=".$storeid."  order by a.reminder desc,a.dateline");//and a.weid = '{$weid}'
        // var_dump($storeid);
        $nowtime = time();
        foreach ($list as $key => $value) {
            $list[$key]['totalprice'] = $list[$key]['total']*$list[$key]['price'];
        ...
        include $this->template($this->cur_tpl . '/lencai');
    }
//配菜系
    public function doMobilePeicai()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        $storeid = $_SESSION['storeid'];
        ...
        $revoke = $_GPC['revoke'];
        if($revoke ==1){
            $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation in (8,13) and a.status=1 and a.stepnow > 0 and a.storeid=".$storeid."  order by a.dateline desc limit 30");//and a.weid = '{$weid}'
        }else{
        // $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.storeid=".$storeid." and a.weid = '{$weid}' and b.pcate in (3,4)  and a.stepnow < 1 and a.status=1 order by a.reminder desc,a.dateline");  // type =2 退菜  and a.type<2 
        $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation in (8,13)  and a.stepnow < 1 and a.status=1 and a.storeid=".$storeid."  order by a.reminder desc,a.dateline");//and a.weid = '{$weid}'
        $nowtime = time();
        foreach ($list as $key => $value) {
            $list[$key]['totalprice'] = $list[$key]['total']*$list[$key]['price'];
            $cha = $nowtime - $list[$key]['dateline'];
     ...
     //   var_dump($list);
        include $this->template($this->cur_tpl . '/peicai');
    }

    //配菜系 慢
    public function doMobilePeicaiman()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        $storeid = $_SESSION['storeid'];
        ...
        $revoke = $_GPC['revoke'];
        if($revoke ==1){
            $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation=14  and a.status=1 and a.stepnow > 0 and a.storeid=".$storeid."  order by a.dateline desc limit 30");//and a.weid = '{$weid}'
        }else{
        // $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.storeid=".$storeid." and a.weid = '{$weid}' and b.pcate in (3,4)  and a.stepnow < 1 and a.status=1 order by a.reminder desc,a.dateline");  // type =2 退菜  and a.type<2 
        $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE e.operation=14  and a.stepnow < 1 and a.status=1 and a.storeid=".$storeid."  order by a.reminder desc,a.dateline");//and a.weid = '{$weid}'
        $nowtime = time();
        foreach ($list as $key => $value) {
            $list[$key]['totalprice'] = $list[$key]['total']*$list[$key]['price'];
            $cha = $nowtime - $list[$key]['dateline'];
     ...
     //   var_dump($list);
        include $this->template($this->cur_tpl . '/peicaiMan');
    }
 // 登陆后判断 权限  APP首页   
public function doMobileOperalist()
{
        global $_W, $_GPC;
        $logid = $_COOKIE['logid'];
        $storeid = $_GPC['storeid'];
        $store = pdo_fetch('select * from ims_weisrc_dish_stores where id ='.$storeid);
        if(empty($storeid)){
            $storeid = $_GPC['storeid'];
        }
        if ($logid =='') {
        ...
        $user = pdo_fetch("SELECT u.*,g.companyname,g.description,u.pcate FROM ims_j_money_user as u left join ims_j_money_group as g ON u.pcate = g.id  WHERE u.storeid=:storeid and u.useracount = :id", array(':id' => $logid,':storeid' => $storeid));
        // $groupid = pdo_query("SELECT pcate FROM  ims_j_money_user  WHERE useracount ='".$logid."'");
        $groupid = $user['pcate'];
        // var_dump($user);

//财务 列出所有店
 $storelist = pdo_fetchall("SELECT * FROM " . tablename($this->table_stores) . " WHERE weid = :weid", array(':weid' => $_W['uniacid']), 'id');//以id为下标

 
        $erp = pdo_fetch('SELECT c.id,c.name,d.id AS locationId FROM ci_contact AS c JOIN ci_storage AS d ON c.number=d.locationNo WHERE c.name="'.$store['title'].'"');
            isetcookie('realname', $user['realname'], 86400 * 100);//联系人
            isetcookie('storeTitle', $erp['name'], 86400 * 100);//店名
            isetcookie('location', $erp['locationId'], 86400 * 100);
            isetcookie('erpuid', $erp['id'], 86400 * 100);
         
        $operalist = "";
        if($groupid==1 ){
            //收银台
        }elseif($groupid ==2 or $groupid==11) {
            $operalist = ".all,.yuangong,.caipin";//店长,查看全部，员工管理，菜品管理
            $opera = "";
        }elseif($groupid ==3 or $groupid ==12) {
            $opera = "zhuyu";
            $operalist = ".zhu";
        }elseif($groupid =='4' or $groupid==16) {
            $opera = "opera";
            $operalist = ".fu";
        }elseif($groupid ==7) {
            $opera = "lencai";
            ...
            $opera = "niurou";
        }elseif($groupid ==8 or $groupid==13) {
            $opera = "peicai";
            $operalist = ".pei";
        }elseif($groupid ==9) {
            // $operalist = ".pei,.all,.zhu,.fu,.sha,.yan,.yan,.leng,.pei";
        }
        var_dump($operalist);
        include $this->template($this->cur_tpl . '/operalist');
}
//员工查看原材料库存17.2.6 material
public function doMobileMaterial()
{
        global $_W, $_GPC;
        $weid = $this->_weid;
        $logid = $_COOKIE['logid'];
        ...
        $storeid = $_SESSION['storeid'];
        $store = pdo_fetch("SELECT s.id FROM " . tablename($this->table_stores) . " as t join ci_storage as s on t.title=s.name where t.id=".$storeid);
        // var_dump($store);
        $pcate = $_GPC['pcate'];
        $list = pdo_fetchall("SELECT staple FROM " . tablename($this->table_goods) . " as g join " . tablename($this->table_category) . " as c on g.pcate=c.id WHERE c.operation=".$pcate." and g.storeid = ".$storeid);
        // var_dump($list);
        foreach ($list as $key => $value) {
            if(!empty($value['staple'])){
                $data['data'] = iunserializer($value['staple']);
        ...
        // var_dump($goodsid);
        if(!empty($goodsid)){
            $materlist = pdo_fetchall("SELECT g.id,g.name,g.costName,sum(i.qty) as total FROM ci_goods as g join ci_invoice_info as i on g.id=i.invId where i.locationId=".$store['id']." and g.id in($goodsid) group by i.invId order by total ASC");
        }
        
        var_dump($materlist);

        include $this->template($this->cur_tpl . '/material');
}
//员工按日期查看菜品销量17.2.7 sales
public function doMobileSales()
{
        global $_W, $_GPC;
        $weid = $this->_weid;
        $logid = $_COOKIE['logid'];
        ...
        $n = date('d');//当月至今天数
        for($i=1;$i<=$n;$i++){
            $nday[$i]['num'] = $year.'-'.$month.'-'.$i;
            $start = strtotime($year.'-'.$month.'-'.$i);
            $end = $start+86400;
            $nday[$i]['ncount'] = pdo_fetch("SELECT sum(o.total) as total FROM " . tablename($this->table_category) . " as c join " . tablename($this->table_goods) . " as g on g.pcate=c.id join " . tablename($this->table_order_goods) . " as o on g.id=o.goodsid WHERE c.operation=".$pcate." and g.storeid = ".$storeid." and o.type!=2 and o.status in (1,2) and o.dateline between $start and $end");
        } 
        foreach ($nday as $key => $value) {
            $ndays[] =$value; 
        }
        // var_dump($ndays);
        // 上月
        if($month == 1){    //如果当前1月的话，处理下
            ...
            $months = $month-1; //获得上个月月份    
        } 
        $ln = cal_days_in_month(CAL_GREGORIAN, $months, $year);//上个月天数
        for($j=1;$j<=$ln;$j++){
            $lday[$j]['num'] = $year.'-'.$months.'-'.$j;
            $first = strtotime($year.'-'.$months.'-'.$j);
            $last = $first+86400;
            $lday[$j]['ncount'] = pdo_fetch("SELECT sum(o.total) as total FROM " . tablename($this->table_category) . " as c join " . tablename($this->table_goods) . " as g on g.pcate=c.id join " . tablename($this->table_order_goods) . " as o on g.id=o.goodsid WHERE c.operation=".$pcate." and g.storeid = ".$storeid." and o.type!=2 and o.status in (1,2) and o.dateline between $first and $last");
        }
        foreach ($lday as $key => $value) {
            $ldays[] =$value; 
        }
        // var_dump($lday);
        include $this->template($this->cur_tpl . '/sales');
}
//员工查看当日菜品销量17.2.6 dailysale
public function doMobileDailysale()
{
        global $_W, $_GPC;
        $weid = $this->_weid;
        $logid = $_COOKIE['logid'];
        ...
        // 获取时间
        if(!empty($way)){
            $start = strtotime($_GPC['times']);
            $end = strtotime($_GPC['timee']);
        }else{
            $start = strtotime($_GPC['dateday']);
            $end = $start+86400;
        }
        if(empty($start)){
            message("请输入查询日期！");
        ...
        $list = pdo_fetchall("SELECT g.title,sum(o.total) as total,g.unitname FROM " . tablename($this->table_category) . " as c join " . tablename($this->table_goods) . " as g on g.pcate=c.id join " . tablename($this->table_order_goods) . " as o on g.id=o.goodsid WHERE c.operation=".$pcate." and g.storeid = ".$storeid." and o.type!=2 and o.status in (1,2) and o.dateline between $start and $end group by o.goodsid");
        // var_dump($list);
        include $this->template($this->cur_tpl . '/dailysale');
}
//店长成本自填表17.2.7 selfill
public function doMobileSelfill()
{
        global $_W, $_GPC;
        $weid = $this->_weid;
        $logid = $_COOKIE['logid'];
        ...
        if(isset($_GPC['submit'])){
        	...
        	$text = $_GPC['list'];
        	$power = $_GPC['power'];
        	$totalprice = $_GPC['count'];
        	$totalnum = count($power);

        	$list = array();
        	foreach ($power as $key => $value) {
        		$list[$key]['power'] = $value; 
        	}
        	...
        	pdo_insert('weisrc_dish_selfill',$date);
        	$id = pdo_insertid();
        	foreach ($list as $key => $value) {
        		pdo_fetch('INSERT INTO ims_weisrc_dish_selfill_text(weid,storeid,selfillid,title,price,dateline,type) VALUES('.$weid.','.$storeid.','.$id.',"'.$value['text'].'",'.$value['power'].','.$createtime.',1)');
        	}
        	if(!empty($id)){
        		$url = 'index.php?i='.$_W['uniacid'].'&c=entry&do=operalist&m=weisrc_dish';
        		// header("location:$url");
                message('填表成功', $url, 'success');
        	}
        }
        include $this->template($this->cur_tpl . '/selfill');
}
//店长收入自填表17.2.24 inselfill
public function doMobileInselfill()
{
        global $_W, $_GPC;
        $weid = $this->_weid;
        $logid = $_COOKIE['logid'];
        ...
        if(isset($_GPC['submit'])){
            ...
            $id = pdo_insertid();
            foreach ($list as $key => $value) {
                pdo_fetch('INSERT INTO ims_weisrc_dish_selfill_text(weid,storeid,selfillid,title,price,dateline,type) VALUES('.$weid.','.$storeid.','.$id.',"'.$value['text'].'",'.$value['power'].','.$createtime.',2)');
            }
            if(!empty($id)){
                $url = 'index.php?i='.$_W['uniacid'].'&c=entry&do=operalist&m=weisrc_dish';
                // header("location:$url");
        ...
        include $this->template($this->cur_tpl . '/inselfill');
}
//店长查询利润表17.2.8 profit
public function doMobileProfits()
{
        global $_W, $_GPC;
        $weid = $this->_weid;
        $logid = $_COOKIE['logid'];
        ...
        $storeid = $_SESSION['storeid'];
        // var_dump($storeid);
        $location = pdo_fetch('SELECT c.id FROM ims_weisrc_dish_stores AS s JOIN ci_contact AS c ON s.title=c.name WHERE s.id='.$storeid);
        $year = date('Y');
        $month =date('m');
        // 本月
        $num = date('d');//当月至今天数
        for($i=$num;$i>=1;$i--){
            $day[$i]['num'] = $year.'-'.$month.'-'.$i;
            $start = strtotime($year.'-'.$month.'-'.$i);
            $start = $start+32400; //早上9点开始
            $end = $start+86400;
            $cash = pdo_fetch('SELECT SUM(cash_fee) AS cash FROM ims_j_money_trade WHERE storeid='.$storeid.' AND createtime>='.$start.' and createtime<'.$end);
            $day[$i]['cash'] = $cash['cash']/100;//实收
            $day[$i]['cost'] = pdo_fetch('SELECT SUM(totalAmount) AS cost FROM ci_invoice WHERE uid='.$location['id'].' AND transType=150601 AND UNIX_TIMESTAMP(billDate)>='.$start.' AND UNIX_TIMESTAMP(billDate)<'.$end);//成本
            $day[$i]['selfill'] = pdo_fetch('SELECT SUM(totalprice) AS selfill FROM ims_weisrc_dish_selfill WHERE storeid='.$storeid.' AND type=1 AND dateline>='.$start.' AND dateline<'.$end);//自填成本
            $day[$i]['inselfill'] = pdo_fetch('SELECT SUM(totalprice) AS selfill FROM ims_weisrc_dish_selfill WHERE storeid='.$storeid.' AND type=2 AND dateline>='.$start.' AND dateline<'.$end);//自填收入
            $day[$i]['profit'] = $day[$i]['cash']-$day[$i]['cost']['cost']-$day[$i]['selfill']['selfill']+$day[$i]['inselfill']['selfill'];
        }
        $sumprice=0;
        foreach ($day as $key => $value) {
            $days[] =$value;
            $sumprice += $value['profit'];
        }
        // 上月
        if($month == 1){    //如果当前1月的话，处理下
            $months = 12;
        ...
        // $lns = cal_days_in_month(CAL_GREGORIAN, $months, $years);//上个月天数
        $n = date('Y-m-t', strtotime('-1 month'));//上月最后一天  不要改这里
        $lns = date('d',strtotime($n));//上个月天数
        $starts = strtotime($years.'-'.$months.''-01);
        $ends = $starts+(86400*$lns);
        $cas = pdo_fetch('SELECT SUM(cash_fee) AS cash FROM ims_j_money_trade WHERE storeid='.$storeid.' AND createtime>='.$starts.' and createtime<'.$ends);
        $cashs = $cas['cash']/100;//实收
        $cost = pdo_fetch('SELECT SUM(totalAmount) AS cost FROM ci_invoice WHERE uid='.$location['id'].' AND transType=150601 AND UNIX_TIMESTAMP(billDate)>='.$starts.' AND UNIX_TIMESTAMP(billDate)<'.$ends);//成本
        $selfill = pdo_fetch('SELECT SUM(totalprice) AS selfill FROM ims_weisrc_dish_selfill WHERE storeid='.$storeid.' AND type=1 AND dateline>='.$starts.' AND dateline<'.$ends);//自填成本
        $inselfill = pdo_fetch('SELECT SUM(totalprice) AS selfill FROM ims_weisrc_dish_selfill WHERE storeid='.$storeid.' AND type=2 AND dateline>='.$starts.' AND dateline<'.$ends);//自填收入
        $profit = $cashs-$cost['cost']-$selfill['selfill']+$inselfill['selfill'];
         //上上月
        if($month == 2){    //如果当前2月的话，处理下
            $lmonth = 12;
            $lyear = date('Y')-1;
        }else{
         $lyear = date('Y');
            $lmonth = $month-2; //获得上个月月份    
        } 
        // $lln = cal_days_in_month(CAL_GREGORIAN, $lmonth, $lyear);
        $ln = date('Y-m-t', strtotime('-2 month'));//上月最后一天
        $lln = date('d',strtotime($ln));//上个月天数
        $lstart = strtotime($lyear.'-'.$lmonth.''-01);
        $lend = $lstart+(86400*$lln);
        $casd = pdo_fetch('SELECT SUM(cash_fee) AS cash FROM ims_j_money_trade WHERE storeid='.$storeid.' AND createtime>='.$lstart.' and createtime<'.$lend);
        $lcash = $casd['cash']/100;//实收
        $lcost = pdo_fetch('SELECT SUM(totalAmount) AS cost FROM ci_invoice WHERE uid='.$location['id'].' AND transType=150601 AND UNIX_TIMESTAMP(billDate)>='.$lstart.' AND UNIX_TIMESTAMP(billDate)<'.$lend);//成本
        $lselfill = pdo_fetch('SELECT SUM(totalprice) AS selfill FROM ims_weisrc_dish_selfill WHERE storeid='.$storeid.' AND type=1 AND dateline>='.$lstart.' AND dateline<'.$lend);//自填成本
        $linselfill = pdo_fetch('SELECT SUM(totalprice) AS selfill FROM ims_weisrc_dish_selfill WHERE storeid='.$storeid.' AND type=1 AND dateline>='.$lstart.' AND dateline<'.$lend);//自填收入
        $lprofit = $lcash-$lcost['cost']-$lselfill['selfill']+$linselfill['selfill'];
        include $this->template($this->cur_tpl . '/profits');
}

//沽清
public function doMobileGuqing()
{
        global $_W, $_GPC;
        $id = $_GPC['goodsid'];
        $status = $_GPC['status'];
        $result = pdo_query("UPDATE " . tablename($this->table_goods) . " SET status=".$status." WHERE id=:id", array(':id' => $id));
        $url = 'index.php?i='.$_W['uniacid'].'&c=entry&do=caipin&m=weisrc_dish';
        message('操作成功', $url, 'success');
}
 // 店长 权限
public function doMobileCaipin()
{
        global $_W, $_GPC;
        $logid = $_COOKIE['logid'];
        if ($logid =='') {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_goods) . " WHERE storeid = ".$storeid);
        // var_dump($list);
        // die();
...
public function doMobileJixiao()
{
        ...
        $user = pdo_fetch("SELECT u.*,g.companyname,g.description FROM ims_j_money_user as u left join ims_j_money_group as g ON u.pcate = g.id  WHERE u.useracount = :id", array(':id' => $logid));
        $storeid = $user['storeid'];
        $users = pdo_fetchall("SELECT storeid,useracount,realname from ims_j_money_user where status=1 and storeid=".$storeid);
...
public function doMobileStaff()
{
        ...
        // $groupid = pdo_query("SELECT pcate FROM  ims_j_money_user  WHERE useracount ='".$logid."'");
        $storeid = $user['storeid'];
        $list = pdo_fetchall("SELECT a.id,a.useracount,a.realname,b.companyname from ims_j_money_user a left join ims_j_money_group b on a.pcate = b.id where a.storeid = ".$storeid." order by a.pcate");
        // var_dump($list);
        // die();
        include $this->template($this->cur_tpl . '/staff');
}
//员工编辑
public function doMobileStaff_editor()
{
        global $_W, $_GPC;
        $id = $_GPC['id'];
        $item = pdo_fetch("SELECT a.*,g.companyname from ims_j_money_user as a join ims_j_money_group as g ON a.pcate=g.id WHERE a.id=:id", array(':id' => $id));
        $pcate = pdo_fetchall("SELECT id,companyname from ims_j_money_group");
        if($item['discount'] ==""){
            $item['discount'] ='0';
        }
        ...
        if($_GPC['edit']){
                ...
                message("修改完成", $this->createMoblieUrl('staff'), 'success');
                }else{
                    //新增
                }
        }
        
        include $this->template($this->cur_tpl . '/staff_editor');
}

//员工编辑
public function doMobileStaff_delete()
{
        global $_W, $_GPC;
        $id = $_GPC['id'];
        $delete = pdo_query("DELETE FROM ims_j_money_user where id=".$id);
        $url = 'index.php?i='.$_W['uniacid'].'&c=entry&do=staff&m=weisrc_dish';
        message('操作成功', $url, 'success');
}

// 取消订单
    public function doMobileCancelorder2() {
        global $_GPC, $_W;
        $orderid = intval($_GPC['orderid']);
        $tables = intval($_GPC['tables']);
        $select = pdo_fetch("SELECT id FROM " . tablename('weisrc_dish_order') . " WHERE status=1 AND tables =".$tables);
        ...
        if(empty($select)){
            pdo_update("weisrc_dish_tables",array('status'=>0),array("id"=>$tables));
        }
...
//  确认订单
    public function doMobileConfirmorder() {
        ...
        if(!empty($select) && $select['id'] != $orderid){
            ...
            // $goods=pdo_delete('weisrc_dish_order',array('id'=>$orderid));//删除加菜订单号
        }else{
            pdo_update('weisrc_dish_order_goods',array('status'=>1),array('orderid'=>$orderid));
    ...
    public function doMobileIsRefesh() {
        global $_GPC, $_W; 
        die();
        $storeid = $_GPC['storeid'];
        $groupid = $_GPC['groupid'];//2服务员， 3厨师
        ...
        //var_dump($groupid);
            // $catid ="8"; // $title = "只看配菜间";
            // $catid ="7"; // $title = "只看冷菜间";
            // $catid ="3,7,8";// 4 服务员 3厨师
        if($groupid==4){
            //echo 'fuwuyuan';
            $chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id WHERE ((e.operation=4 and a.stepnow = 0) or (e.operation not in (4) and a.stepnow=1)) and a.status=1 and a.storeid=".$storeid);
            if($chu['num'] != $chu0){
                setcookie("chu",$chu['num'],time()+3600*24);
                die('1');
            }
        }
        if($groupid==3){
            //echo 'chu';
            $chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id WHERE (e.operation in (3) and a.stepnow=0) and a.status=1 and a.storeid=".$storeid);
            if($chu['num'] != $chu0){
                setcookie("chu",$chu['num'],time()+3600*24);
                die('1');
            }
        ...
        if($groupid==7){
            //echo 'lengcai';
            $chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id WHERE (e.operation in (7) and a.stepnow=0) and a.status=1 and a.storeid=".$storeid);
            if($chu['num'] != $leng0){
                setcookie("leng",$chu['num'],time()+3600*24);
                die('1');
            }
        }
        if($groupid==8){
            //echo 'pei';
            $chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id WHERE (e.operation in (8) and a.stepnow=0) and a.status=1 and a.storeid=".$storeid);
            if($chu['num'] != $pei0){
                setcookie("pei",$chu['num'],time()+3600*24);
                die('1');
            }
        }
    }
//  刷新APP  doMobileOperalist
    public function doMobileRefeshlist() {
        global $_GPC, $_W; 
        $refesh =0;
        $storeid = $_GPC['storeid'];
        $groupid = $_GPC['groupid'];//2服务员， 3厨师
        ...
        if($groupid==4 or $groupid==16){
            //echo 'fuwuyuan';
            $chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id WHERE ((e.operation in (4,16) and a.stepnow = 0) or (e.operation not in (4,16) and a.stepnow=1)) and a.status=1 and a.storeid=".$storeid);
                setcookie("chu",$chu['num'],time()+3600*24);
            if($chu['num'] != $chu0){
                $refesh =1;
            }
        }
        if($groupid==3 or $groupid==12){
            //echo 'chu';
            $chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id WHERE (e.operation in (3,12) and a.stepnow=0) and a.status=1 and a.storeid=".$storeid);
                setcookie("chu",$chu['num'],time()+3600*24);
            if($chu['num'] != $chu0){
                 $refesh =1;
            }
        ...
        if($groupid==15){
            //echo 'niu';
            $chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id WHERE (e.operation in (15) and a.stepnow=0) and a.status=1 and a.storeid=".$storeid);
                setcookie("niu",$chu['num'],time()+3600*24);
            if($chu['num'] != $niu0){
                 $refesh =1;
            }
        }
        if($groupid==8 or $groupid==13){
            //echo 'pei';
            $chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id WHERE (e.operation in (8,13) and a.stepnow=0) and a.status=1 and a.storeid=".$storeid);
                setcookie("pei",$chu['num'],time()+3600*24);
            if($chu['num'] != $pei0){
                 $refesh =1;
            }
        ...
        if($groupid==14){
            //echo 'pei man';
            $chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id WHERE (e.operation in (14) and a.stepnow=0) and a.status=1 and a.storeid=".$storeid);
                setcookie("peim",$chu['num'],time()+3600*24);
            if($chu['num'] != $peim0){
                 $refesh =1;
            }
        }
        include $this->template($this->cur_tpl . '/refeshlist');
    }
public function doMobilePlay() {//播放声音
        global $_GPC, $_W;
        include $this->template($this->cur_tpl . '/play');
    }
...
public function doMobileOpera()
    {
        ...
        $opera['storeid'] = $_GPC['storeid'];
        $opera['tablesid'] = $_GPC['tablesid'];
        $opera['pcate'] = $_GPC['pcate'];
        $opp = $_GPC['opp'];
        $cat = $_GPC['cat'];
        $zone = $_GPC['zone'];
        $time = time();
        $ygname = $_SESSION['realname'];
       // var_dump($ygname);
        $logid = $_COOKIE['logid'];
        //有新订单来了
        $newOrder= pdo_fetch("select a.id,a.tables,b.title from " . tablename($this->table_order) . " a left join ".tablename($this->table_tables)." b on a.tables =b.id where a.storeid=".$storeid." and a.status=0 limit 1");
        if ($logid =='') {
            $urll = 'index.php?i='.$_W['uniacid'].'&c=entry&do=index&m=j_money';
           message('未登录', $urll, 'success');
        }
             ...
             //按楼层
        $tablelist =pdo_fetchall("SELECT * from ims_weisrc_dish_tablezones a WHERE a.storeid = ".$storeid." order by a.order");
        foreach($tablelist as $key => $value){
            $tablelist[$key]['goods'] = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.weid = '{$weid}' and (e.operation in (4,16) and a.stepnow = 0)  and e.id!=9 and a.status=1 and a.storeid=".$storeid." and c.id = ".$value['id']." order by a.reminder desc,a.dateline");//(or (e.operation in (3,7,8) and a.stepnow=1))
            }
        //end

        if($zone){
            $zoneOnly =" and c.id =".$zone;
        }
        if($cat =="pei"){
            $catid ="8,13,14";
            $title = "只看配菜间";
            ...
            $catid ="3,7,8,13,14,16";
            $title = "上菜列表";
        }
        //只看分类
$pei = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.weid = '{$weid}' and (e.operation  in (8,13,14)  and a.stepnow=1) and e.id!=9 and a.status=1 and a.storeid=".$storeid.$zoneOnly."  order by a.reminder desc,a.dateline");
$niurou = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.weid = '{$weid}' and (e.operation  in (15)  and a.stepnow=1) and e.id!=9 and a.status=1 and a.storeid=".$storeid.$zoneOnly."  order by a.reminder desc,a.dateline");
$leng = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.weid = '{$weid}' and (e.operation in (7) and a.stepnow=1) and e.id!=9 and a.status=1 and a.storeid=".$storeid.$zoneOnly."  order by a.reminder desc,a.dateline");
$chu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.weid = '{$weid}' and (e.operation in (3,12) and a.stepnow=1) and e.id!=9 and a.status=1 and a.storeid=".$storeid.$zoneOnly."  order by a.reminder desc,a.dateline");// 
$fu = pdo_fetch("SELECT count(*) as num FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.weid = '{$weid}' and (e.operation in (4,16) and a.stepnow = 0) and  a.status=1 and a.storeid=".$storeid.$zoneOnly."  order by a.reminder desc,a.dateline");
        //end
       
  
       if(!empty($cat)){
       $list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.weid = '{$weid}' and (e.operation in (".$catid.") and a.stepnow=1) and e.id!=9 and a.status=1 and a.storeid=".$storeid.$zoneOnly."  order by a.reminder desc,a.dateline");
       }elseif($zone){
$list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.weid = '{$weid}' and (e.operation in (4,16) and a.stepnow = 0)  and e.id!=9 and a.status=1 and a.storeid=".$storeid.$zoneOnly."  order by a.reminder desc,a.dateline");

       }else{
$list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname,a.taste as flavor FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join ".tablename($this->table_category)." as e on b.pcate=e.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id  WHERE a.weid = '{$weid}' and ((e.operation in (4,16) and a.stepnow = 0) or (e.operation in (".$catid.") and a.stepnow=1)) and e.id!=9 and a.status=1 and a.storeid=".$storeid.$zoneOnly."  order by a.reminder desc,a.dateline");
        ...
        include $this->template($this->cur_tpl . '/opera');
    }

    //所有订单
public function doMobileOrderall()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        //$from_user = $_W['member']['mobile']; 
...
$list = pdo_fetchall("SELECT a.*,b.thumb,b.title,b.pcate,c.title as sortname,d.title as tablesname FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  left join " . tablename($this->table_tablezones) . " as c on d.tablezonesid=c.id WHERE a.weid = '{$weid}' and a.dateline >=$start and a.dateline <$end and a.status=1 and a.type <2 and b.storeid=".$opera['storeid']." order by a.reminder desc,a.dateline");  //and a.stepnow= {$opera['step']}  and a.stepnow >0
        
        foreach ($list as $key => $value) {
            $list[$key]['totalprice'] = $list[$key]['total']*$list[$key]['price'];
            $cha = $nowtime - $list[$key]['dateline'];
            if ($cha >300 && $cha <600 && $value['reminder']==0 && $value['stepnow']<5) {
                $list[$key]['status'] = "warming";
                $list[$key]['tips'] = "10分钟内";
            }elseif ($cha >600 && $value['reminder']==0 && $value['stepnow']<5) {
                $list[$key]['tips'] = "超过10分钟了";
                $list[$key]['status'] = "danger";
            }elseif($value['reminder']==1 && $value['stepnow']<5){
                $list[$key]['tips'] = "用户崔单了";
                $list[$key]['status'] = "blue";
            }else{
       ...
       //  $renyuan = pdo_fetchall("SELECT * FROM renyuan where logintime between $start and $end group by name,name  order by identity");
        
       // $login = array();
       // foreach($renyuan as $logins){
       // $login[] = $logins;
       // }
        include $this->template($this->cur_tpl . '/orderall');
    }


    //按桌号查看订单
public function doMobileLook_tables()
    {
        global $_W, $_GPC;
        $storeid = $_SESSION['storeid'];
        if ($logid =='') {
...
// 按桌号查看订单
        $tableAll = pdo_fetchall("SELECT * from ims_weisrc_dish_tables where storeid=".$storeid." and status>=1 order by tablezonesid desc,title ASC");
        foreach($tableAll as $key => $value){
            $tableAll[$key]['name']=pdo_fetch("SELECT title from ims_weisrc_dish_tablezones where id=".$value['tablezonesid']);
            $tableAll[$key]['goods']=pdo_fetch("SELECT count(*) as num from " . tablename($this->table_order_goods) . " where type=0 and status in (0,1) and stepnow in (0,1) and  tablesid=".$value['id']);
        }
// 按桌号查看订单end

    include $this->template($this->cur_tpl . '/look_tables');
    }

    //按桌号查看订单列表 详情
public function doMobileLook_list()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        $storeid = $_SESSION['storeid'];
        ...
        // var_dump($orderid);
        if(!$orderid){
             // $order = pdo_fetch("SELECT * from ims_weisrc_dish_order WHERE tables = ".$tables."  and status in(0,1) and ispay =0 order by id desc  LIMIT 1");
            $orders = pdo_fetch('SELECT * from ims_weisrc_dish_order WHERE storeid='.$storeid.' and status=0 and ispay = 0 and tables ='.$tables.' order by id ASC limit 1');
            // var_dump($orders);
            $order = pdo_fetch("SELECT * from ims_weisrc_dish_order WHERE storeid=".$storeid." and tables = ".$tables."  and status in(1) and ispay =0 order by id ASC  LIMIT 1");
            $orderid = $order['id'];      
        }
        ...
        // var_dump($orderid);
        if($orders){
          $lists = pdo_fetchall("SELECT a.title,b.taste,a.pcate,b.* from ims_weisrc_dish_goods as a join ims_weisrc_dish_order_goods as b on a.id=b.goodsid where b.storeid=".$storeid." and b.status=0 and b.orderid=".$orders['id']." order by b.type,b.stepnow,a.title");
          foreach($lists as $key=>$value){
                $lists[$key]['addprice'] = $value['price']*$value['total'];
            }
            $addprice = $orders["totalprice"];
        }
        if($orderid){
            $order = pdo_fetch("SELECT * from ims_weisrc_dish_order WHERE storeid=".$storeid." and id = ".$orderid." LIMIT 1 ");
            $list = pdo_fetchall("SELECT a.title,b.taste,a.pcate,b.* from ims_weisrc_dish_goods as a join ims_weisrc_dish_order_goods as b on a.id=b.goodsid where b.storeid=".$storeid." and b.status in (0,1) and b.orderid=".$orderid." order by b.type,b.stepnow,a.title");
            foreach($list as $key=>$value){
                $list[$key]['oneprice'] = $value['price']*$value['total'];
            }
            $totalprice = pdo_fetch("SELECT sum(total*price) as total from ".tablename('weisrc_dish_order_goods')." WHERE (status =0 or status =1) and type =0 and storeid=".$storeid." and orderid=".$orderid);
            $totalprice= round($totalprice['total'],0);
            // var_dump($list);
            //$totalprice = pdo_fetch("SELECT SUM(price*total) as totalprice from ims_weisrc_dish_order_goods where status=1 and tablesid=".$tables);
            // $totalprice = $order["totalprice"];
                $zone = pdo_fetch("select b.title from  " . tablename($this->table_tablezones) . " b  left join " . tablename($this->table_tables) . " a on a.tablezonesid  = b.id where a.storeid=".$storeid." and a.id = ".$order['tables']);
            $zone = $zone['title'];
        ...
        //     $tips = "用户未确认订单！"; 
        // }
    include $this->template($this->cur_tpl . '/look_list');
    }
    ...
    //         $sql=pdo_fetch("select uid, mobile, pwd, nickname,identity from ims_weisrc_dish_account where mobile=". $mobile ." and pwd='". $password."'");
    //         if($sql==false){
    //             echo "<script language='javascript'>alert('您输入的信息有误，请重新输入!');history.back();</script>";
    //             exit;
    //         }else{
    //         //session_start();
    //          $_SESSION['yuangong']=$sql['uid'];
    //          $_SESSION['ygname']=$sql['nickname'];
    //          $_SESSION['identity']=$sql['identity'];
    //          setcookie("mobile",$mobile,time()+3600*24);
    //          setcookie("password",$password,time()+3600*24);
    //          setcookie("identity",$sql['identity'],time()+3600*24);
    //          $insert=pdo_fetch("insert into renyuan(name,logintime,uid,identity) values('".$_SESSION['ygname']."',".$date.",".$_SESSION['yuangong'].",".$_SESSION['identity'].")");
    //             if($_SESSION['identity']==1){
    //                 message('登录成功', $this->createMobileUrl('fish'), 'success');
    //             exit;
    //             }else{
    //                 message('登录成功', $this->createMobileUrl('opera'), 'success');
    //                 exit;
    //             }
    ...
    //     if(isset($_COOKIE['identity']) && $_COOKIE['identity']==1) {
    //         //message('欢迎回来！', $this->createMobileUrl('fish'), 'success');

    //         $back_url ='index.php?i='.$_W['uniacid'].'&c=entry&do=fish&m=weisrc_dish';
    //         header("location:$back_url");
    //     }else{

    //         $back_url ='index.php?i='.$_W['uniacid'].'&c=entry&do=opera&m=weisrc_dish';
    //         header("location:$back_url");
    //     }
    //     $mobile = $_COOKIE['mobile'];
    ...
    //     include $this->template($this->cur_tpl . '/login');
    // }
    public function doMobilelogin(){
        session_start();
        global $_GPC, $_W;;
        ...
        $logid = $_GPC['logid'];
        if($logid!=''){
            $item=pdo_fetch("SELECT * FROM ims_j_money_user WHERE weid=$weid and useracount=".$logid);
            $_SESSION['pcate']=$item['pcate'];
            $_SESSION['realname']=$item['realname'];
            $_SESSION['storeid'] = $item['storeid'];
            setcookie("pcate",$item['pcate'],time()+3600*24);
            setcookie("logid",$logid,time()+3600*24);
            if($item['pcate']==3){
                $log_url ='index.php?i='.$_W['uniacid'].'&c=entry&do=operalist&m=weisrc_dish';
            ...
            //$out_url ='http://shanghai.xyj0772.com/app/index.php?i='.$_W['uniacid'].'&c=entry&do=index&m=j_money';
            message('登录失败！');
        }
...
//退出登录
    public function doMobilelogout()
    {
        ...
        setcookie("pcate",'');
        setcookie("logid",'');
        $out_url ='index.php?i='.$_W['uniacid'].'&c=entry&do=index&m=j_money';
        header("location:$out_url");
    }
public function doMobileOperafish()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        $id = $_GPC['id'];
        ...
        $goodsid = $_GPC['goodsid'];
        $weight = $_GPC['weight'];
        $stepnow = $_GPC['stepnow'];
        //修改鱼表
        $result = pdo_query("UPDATE ims_weisrc_dish_fish SET done=".$stepnow.",finishtime=".time()." WHERE id=:id", array(':id' => $id));
        //var_dump($stepnow);
        if($stepnow ==5){
            pdo_delete('weisrc_dish_fish', array('id' => $id));
        }
        //修改订单表
        $ygname = $_SESSION['realname'];
        if($stepnow ==0){
            $result2 = pdo_query("UPDATE ims_weisrc_dish_order_goods SET stepnow=".$stepnow." WHERE tablesid=".$tablesid." and goodsid=".$goodsid." and stepnow =1 and total =".$weight);
        }else{
        $result2 = pdo_query("UPDATE ims_weisrc_dish_order_goods SET stepnow=".$stepnow." WHERE tablesid=".$tablesid." and goodsid=".$goodsid." and stepnow <".$stepnow." and total =".$weight);
    }
        // $result2 = pdo_query("UPDATE ims_weisrc_dish_order_goods SET stepnow=".$stepnow." WHERE tablesid=".tablesid." and total='".$weight."'");
       // var_dump($result2); 
        $url = $_SERVER["HTTP_REFERER"];// 获得来路URL
        // message('操作成功！', $url, 'success');
        header("location:$url");
}
public function doMobileOperadetail()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        //$from_user = $_W['member']['mobile']; 
        ...
        if(empty($ids)){
            $url = $_SERVER["HTTP_REFERER"];// 获得来路URL
// header("location:$url");
            message('未收到菜单，请选择菜单！', $url, 'success');
        }
        // var_dump($ids);
        // var_dump($_GPC['stepnow']);
        // die();

        
//按扫描次数判断操作
if($_GPC['stepnow']==4){//上菜
    $resule = pdo_query("UPDATE " . tablename($this->table_order_goods) . " SET stepnow=5,".$step."=".time().",".$stepman."='".$ygname."' WHERE id in (".$ids.")");
}
elseif($_GPC['stepnow']==2){//撤销操作
    $resule = pdo_query("UPDATE " . tablename($this->table_order_goods) . " SET stepnow=0,".$step."=".time().",".$stepman."='".$ygname."' WHERE id in (".$ids.")");
       // var_dump($resule);
       //  die();
}
else{//后厨
    $resule = pdo_query("UPDATE " . tablename($this->table_order_goods) . " SET stepnow=1,".$step."=".time().",".$stepman."='".$ygname."' WHERE id in (".$ids.")");
}
...
public function doMobileReminder()
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        //$from_user = $_W['member']['mobile']; 
...
//判断催菜操作 
$resule = pdo_query("UPDATE " . tablename($this->table_order_goods) . " SET reminder=1 WHERE id=:id", array(':id' => $orderid));
$url = $_SERVER["HTTP_REFERER"];// 获得来路URL
 message('催菜成功！', $url, 'success');
    ...
    public function doMobileWapSelect()
    {
        ...
        $authurl = $_W['siteroot'] . 'app/' . $this->createMobileUrl($method, array('storeid' => $storeid,'tablesid' => $tablesid,'mode' => $mode), true) . '&authkey=1';
        $url = $_W['siteroot'] . 'app/' . $this->createMobileUrl($method, array('storeid' => $storeid,'tablesid' => $tablesid,'mode' => $mode), true);
        if (isset($_COOKIE[$this->_auth2_openid])) {
            $from_user = $_COOKIE[$this->_auth2_openid];
            $nickname = $_COOKIE[$this->_auth2_nickname];
            $headimgurl = $_COOKIE[$this->_auth2_headimgurl];
    ...
    public function doMobileWapSelectList()
    {
        ...
        //读取相关产品
        $goods = pdo_fetchall("SELECT * FROM " . tablename($this->table_goods) . " WHERE FIND_IN_SET(title, '{$intelligent['content']}') AND weid=:weid AND storeid=:storeid", array(':weid' => $weid, ':storeid' => $storeid));
// var_dump($goods);
        $total_money = 0;
        foreach ($goods as $key => $value) {
    ...
    public function doMobileAddToMenu()
    {
        ...
        if (!empty($intelligent)) {
            $goods = pdo_fetchall("SELECT * FROM " . tablename($this->table_goods) . " WHERE FIND_IN_SET(title, '{$intelligent['content']}') AND weid={$weid} AND storeid={$storeid}");//FIND_IN_SET(id, '{$intelligent['content']}') 17.3.3
        ...
        $url = $this->createMobileUrl('waplist', array('storeid' => $storeid,'tablesid' => $tablesid,'mode' => $mode), true);//17.3.3
...
//        die('<script>location.href = "' . $url . '";</script>');
    ...
    public function doMobileAddToOrder()
    {
        ...
        //查询购物车
        $cart = pdo_fetchall("SELECT * FROM " . tablename($this->table_cart) . " WHERE weid = :weid AND (from_user = :from_user or tablesid= :tables) AND storeid=:storeid order by id DESC", array(':weid' => $weid, ':from_user' => $from_user, ':storeid' => $storeid, ':tables' => $tables));
        //修改购物车所有鱼可下单, 'goodsid'
        // var_dump($cart);
        // die();
        if ($rtype != 1) {
        ...
        foreach ($cart as $value) {
            ...
            $goodsprice = $goodsprice + (floatval($value['total']) * floatval($value['price']));
        }
       ...
       // die();
        foreach ($cart as $row) {
           ...
           $tasteid =  $row['goodsid'];
            if (!empty($_GPC["goodsid".$tasteid])) {
                $taste = $_GPC["goodsid".$tasteid];
            }else{
                $taste = $row['taste'];
            }
            ...
            pdo_insert($this->table_order_goods, array(
                ...
                'taste' => $taste,
                // 'taste' => $row['taste'],
                // 'taste' => $_GPC['"'.$row['goodsid'].'"'],
            ));
        }
        if ($rtype != 1) {
            pdo_delete($this->table_cart, array('weid' => $weid, 'from_user' => $from_user, 'storeid' => $storeid));
            pdo_delete($this->table_cart, array('weid' => $weid, 'tablesid' => $tables, 'storeid' => $storeid));
    ...
    public function payResult($params)
    {
        ...
        $paytype = array('credit' => '1', 'wechat' => '2', 'alipay' => '2', 'delivery' => '3', 'queren' => '4');
        ...
        if (!empty($order)) {
            ...
            if ($order['dining_mode'] == 1) {
                if ($data['paytype'] == 3) { //现金
                    ...
                    pdo_update($this->table_tables, array('status' => 1), array('id' => $order['tables']));
...
//                $this->sendOrderSms($order);
        ...
        if ($params['type'] == $credit) {
            ...
            message('订单已确认！', $this->createMobileUrl('orderdetail', array('orderid' => $orderid), true), 'success');
        }else {
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
        ...
        $title = $this->actions_titles[$action];
        // $storeid = intval($_GPC['storeid']);
        $storeid = intval($_GET['storeid']);//修改获取方法17/1/17
        if (empty($storeid)) {
            message('请选择门店!');
        }
...
//            $user['groupid'] = intval($_GPC['groupid']) ? intval($_GPC['groupid']) : message('请选择所属用户组');
            ...
            $account =pdo_fetchall("SELECT id,nickname,identity from ims_weisrc_dish_account order by identity");

            foreach($account as $key=>$steps){
                $stepsum=0;
                for($i=1;$i<=5;$i++){  
                $step='step'.$i.'man';
                $account[$key]['stepcount'] = pdo_fetch("SELECT COUNT($step) as count from ims_weisrc_dish_order_goods where $step='".$steps['nickname']."' and PERIOD_DIFF(date_format(now(), '%Y%m') , date_format(FROM_UNIXTIME(dateline), '%Y%m'))=1"); 
                $stepsum+=$account[$key]['stepcount']['count'];
                }
                $account[$key]['num']=$stepsum;
            }

            foreach($account as $key=>$stepsn){
                $stepsumn=0;
                for($i=1;$i<=5;$i++){  
                $step='step'.$i.'man';
                $account[$key]['stepcountn'] = pdo_fetch("SELECT COUNT($step) as count from ims_weisrc_dish_order_goods where $step='".$stepsn['nickname']."' and DATE_FORMAT(FROM_UNIXTIME(dateline),'%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m')");  
                $stepsumn+=$account[$key]['stepcountn']['count'];
                }
                $account[$key]['nums']=$stepsumn;
            }
            // var_dump($account); 
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
        ...
        $returnid = $this->checkPermission($storeid);
        //添加分类操作员17.1.18
        $ation = pdo_fetchall("SELECT id,companyname FROM ims_j_money_group");
        //end
        $operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
        ...
        if ($operation == 'display') {
            ...
            $children = array();
            $category = pdo_fetchall("SELECT * FROM " . tablename($this->table_category) . " WHERE weid = '$weid'  AND storeid ={$storeid} ORDER BY id ASC, displayorder DESC");
            //16.12.2
            // foreach ($category as $index => $row) {
            //     if (!empty($row['parentid'])) {
            //         $children[$row['parentid']][] = $row;
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
        ...
        if ($operation == 'post') {
            ...
            $store = pdo_fetch("SELECT * FROM " . tablename($this->table_stores) . " WHERE id = :id LIMIT 1", array(':id' => $storeid));
            $logo = tomedia($store['logo']);
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_tables) . " WHERE weid = :weid AND storeid =:storeid {$condition} ORDER BY tablezonesid DESC, title ASC", array(':weid' => $this->_weid, ':storeid' => $storeid));

            foreach ($list as $key => $value) {
                    $list[$key]['totalprice']= pdo_fetch("SELECT totalprice FROM  " . tablename($this->table_order) . "  WHERE tables = ".$value['id']." order by id desc");
                    $list[$key]['type']= pdo_fetch("SELECT title from " . tablename($this->table_tablezones)." where id=".$value['tablezonesid']);
                }
                 // var_dump($list);
                 // die();
...
//            message($status);
            ...
            message('操作成功！', $this->createWebUrl('tables', array('op' => 'display', 'storeid' => $storeid)), 'success');
        } else if ($operation == 'changetable') {
            $tablesid = intval($_GPC['tablesid']);
            $tablesnet = intval($_GPC['change']);
            $status = intval($_GPC['status']);

            pdo_update($this->table_tables, array('status' => 0), array('id' => $tablesid, 'weid' => $weid,'storeid' => $storeid));
            pdo_update($this->table_tables, array('status' => $status), array('id' => $tablesnet, 'weid' => $weid,'storeid' => $storeid));
            pdo_update($this->table_order, array('tables' => $tablesnet), array('tables' => $tablesid, 'weid' => $weid,'storeid' => $storeid));
            $sql=pdo_fetch("SELECT id from " . tablename($this->table_order) . " where tables=".$tablesnet." order by id desc");
            pdo_update($this->table_order_goods, array('tablesid' => $tablesnet), array('orderid' => $sql['id'], 'weid' => $weid,'storeid' => $storeid));
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
        ...
        if ($operation == 'post') {
            ...
            if (!empty($id)) {
                $item = pdo_fetch("SELECT * FROM " . tablename($this->table_tables) . " WHERE id = :id", array(':id' => $id));
              //  $item = pdo_fetch("SELECT a.*,b.totalprice,b.tables FROM " . tablename($this->table_tables) . " as a join " . tablename($this->table_order) . " as b on a.id=b.tables WHERE a.id = :id order by b.meal_time", array(':id' => $id));
            ...
            if (checksubmit('submit')) {
                ...
                message('操作成功！', $this->createWebUrl('tables2', array('op' => 'display', 'storeid' => $storeid)), 'success');
            }
        } else if ($operation == 'display') {
            $condition = '';
            if (!empty($_GPC['keyword'])) {
            ...
            foreach ($list as $key => $value) {
                    $list[$key]['totalprice']= pdo_fetch("SELECT totalprice FROM  " . tablename($this->table_order) . "  WHERE tables = ".$value['id']." order by id desc");
                    $list[$key]['snid'] = pdo_fetch("SELECT id FROM  " . tablename($this->table_order) . "  WHERE tables = ".$value['id']." order by id desc");
                    $list[$key]['type']= pdo_fetch("SELECT title from " . tablename($this->table_tablezones)." where id=".$value['tablezonesid']);
                }
            ...
            pdo_delete($this->table_tables, array('id' => $id, 'weid' => $weid));
            message('操作成功！', $this->createWebUrl('tables2', array('op' => 'display', 'storeid' => $storeid, 'type' => 'qrcode')), 'success');
        } else if ($operation == 'updatestate') {
            $tablesid = intval($_GPC['tablesid']);
            $status = intval($_GPC['workflow_state']);
//            message($status);
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
 ...
 public function doWebMaterials()
    {
        global $_GPC, $_W;
//        $GLOBALS['frames'] = $this->getNaveMenu();
        $weid = $this->_weid;
        ...
        $operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
        $tablezones = pdo_fetchall("SELECT * FROM " . tablename($this->table_materialzones) . " WHERE weid = :weid AND storeid=:storeid ORDER BY displayorder DESC", array(':weid' => $weid, ':storeid' => $storeid));

        if (empty($tablezones)) {
            $url = $this->createWebUrl('materialzones', array('op' => 'display', 'storeid' => $storeid));
            message('请先添加类型', $url);
        }
        ...
        if ($operation == 'post') {
            ...
            if (!empty($id)) {
                $item = pdo_fetch("SELECT * FROM " . tablename($this->table_materials) . " WHERE id = :id", array(':id' => $id));
                if (empty($item)) {
                    message('抱歉，数据不存在或是已经删除！', '', 'error');
                }
            }
            ...
            if (checksubmit('submit')) {
                ...
                if (empty($id)) {
                    pdo_insert($this->table_materials, $data);
                } else {
                    unset($data['dateline']);
                    pdo_update($this->table_materials, $data, array('id' => $id, 'weid' => $weid));
                }
                message('操作成功！', $this->createWebUrl('materials', array('op' => 'display', 'storeid' => $storeid)), 'success');
            }
        } else if ($operation == 'detail') {
            $tablesid = intval($_GPC['tablesid']);
            $item = pdo_fetch("SELECT * FROM " . tablename($this->table_materials) . " WHERE id = :id", array(':id' => $tablesid));
            $cate = pdo_fetch("SELECT * FROM " . tablename($this->table_materialzones) . " WHERE id = :id", array(':id' => $item['tablezonesid']));
            $store = pdo_fetch("SELECT * FROM " . tablename($this->table_stores) . " WHERE id = :id", array(':id' => $item['storeid']));
            $logo = tomedia($store['logo']);
            $tablesorder = pdo_fetchcolumn("SELECT count(1) AS count FROM " . tablename($this->table_tables_order) . " where weid = :weid AND storeid =:storeid AND tablesid=:tablesid ", array(':weid' => $this->_weid, ':storeid' => $storeid, ':tablesid' => $tablesid));
            $tablesorderuser = pdo_fetchcolumn("SELECT count(distinct(from_user)) AS count FROM " . tablename($this->table_tables_order) . " where weid = :weid AND storeid =:storeid AND tablesid=:tablesid ", array(':weid' => $this->_weid, ':storeid' => $storeid, ':tablesid' => $tablesid));
            ...
            $store = pdo_fetch("SELECT * FROM " . tablename($this->table_stores) . " WHERE id = :id LIMIT 1", array(':id' => $storeid));
            $logo = tomedia($store['logo']);
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_materials) . " WHERE weid = :weid AND storeid =:storeid {$condition} ORDER BY displayorder DESC, id DESC", array(':weid' => $this->_weid, ':storeid' => $storeid));

            $tablezones = pdo_fetchall("SELECT id,title FROM " . tablename($this->table_materialzones) . " where weid = :weid AND storeid =:storeid ", array(':weid' => $this->_weid, ':storeid' => $storeid), 'id');

            $tablesorder = pdo_fetchall("SELECT tablesid,count(1) AS count FROM " . tablename($this->table_tables_order) . " where weid = :weid AND storeid =:storeid GROUP BY tablesid ", array(':weid' => $this->_weid, ':storeid' => $storeid), 'tablesid');

        } else if ($operation == 'delete') {
            $id = intval($_GPC['id']);
            $row = pdo_fetch("SELECT id FROM " . tablename($this->table_materials) . " WHERE id = :id", array(':id' => $id));
            if (empty($row)) {
                message('抱歉，数据不存在或是已经被删除！');
            }

            pdo_delete($this->table_materials, array('id' => $id, 'weid' => $weid));
            message('操作成功！', $this->createWebUrl('materials', array('op' => 'display', 'storeid' => $storeid, 'type' => 'qrcode')), 'success');
        } else if ($operation == 'updatestate') {
            $tablesid = intval($_GPC['tablesid']);
            $status = intval($_GPC['workflow_state']);
//            message($status);
            pdo_update($this->table_materials, array('status' => $status), array('id' => $tablesid, 'weid' => $weid));
            message('操作成功！', $this->createWebUrl('materials', array('op' => 'display', 'storeid' => $storeid)), 'success');
        }

        include $this->template('materials');
    }

    public function doWebMaterialZones()
    {
        global $_GPC, $_W;
//        $GLOBALS['frames'] = $this->getNaveMenu();
        $weid = $this->_weid;
        ...
        if ($operation == 'post') {
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $item = pdo_fetch("SELECT * FROM " . tablename($this->table_materialzones) . " WHERE id = :id", array(':id' => $id));
                if (empty($item)) {
                    message('抱歉，队列不存在或是已经删除！', '', 'error');
                }
            }
            if (checksubmit('submit')) {
                ...
                if (empty($id)) {
                    pdo_insert($this->table_materialzones, $data);
                } else {
                    unset($data['dateline']);
                    pdo_update($this->table_materialzones, $data, array('id' => $id, 'weid' => $weid));
                }
                message('操作成功！', $this->createWebUrl('materialzones', array('op' => 'display', 'storeid' => $storeid)), 'success');
            }
        } else if ($operation == 'display') {
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_materialzones) . " WHERE weid = :weid AND storeid =:storeid ORDER BY displayorder DESC", array(':weid' => $this->_weid, ':storeid' => $storeid));

            $stores = pdo_fetchall("SELECT id,title FROM " . tablename($this->table_stores) . " where weid = :weid ", array(':weid' => $this->_weid), 'id');
            $table_count = pdo_fetchall("SELECT tablezonesid,COUNT(1) as count FROM " . tablename($this->table_materials) . " where storeid=:storeid AND weid = :weid GROUP BY tablezonesid", array(':weid' => $this->_weid, ':storeid' => $storeid), 'tablezonesid');
        } else if ($operation == 'delete') {
            $id = intval($_GPC['id']);
            $row = pdo_fetch("SELECT id FROM " . tablename($this->table_materialzones) . " WHERE id = :id", array(':id' => $id));
            if (empty($row)) {
                message('抱歉，数据不存在或是已经被删除！');
            }

            pdo_delete($this->table_materialzones, array('id' => $id, 'weid' => $weid));
            message('操作成功！', $this->createWebUrl('materialzones', array('op' => 'display', 'storeid' => $storeid)), 'success');
        }
        include $this->template('material_zones');
    }
    //添加做法
 public function doWebTaste()
    {
        global $_GPC, $_W;
//        $GLOBALS['frames'] = $this->getNaveMenu();
        $weid = $this->_weid;
        ...
        $operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
        if ($operation == 'post') {
            $list=pdo_fetchall("SELECT * FROM ims_weisrc_dish_goods_taste");

            if (checksubmit('submit')) {
                $data = array(
                    'name' => trim($_GPC['title']),
                    'detail' => trim($_GPC['detail']),
                    'createtime' => TIMESTAMP,
                );

                pdo_insert('weisrc_dish_goods_taste', $data);
                message('操作成功！', $this->createWebUrl('taste', array('op' => 'post', 'storeid' => $storeid)), 'success');
            }
        }else if ($operation == 'delete') {
            $id = intval($_GPC['id']);
            $row = pdo_fetch("SELECT id FROM ims_weisrc_dish_goods_taste WHERE id = :id", array(':id' => $id));
            if (empty($row)) {
                message('抱歉，数据不存在或是已经被删除！');
            }

            pdo_delete('weisrc_dish_goods_taste', array('id' => $id));
            message('操作成功！', $this->createWebUrl('taste', array('op' => 'post', 'storeid' => $storeid)), 'success');
        }
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
    ...
    //收银台队列管理
     public function doWebQueueSetting2()
    {
        global $_GPC, $_W;
//        $GLOBALS['frames'] = $this->getNaveMenu();
        $weid = $this->_weid;
        ...
        if ($operation == 'post') {
            ...
            if (checksubmit('submit')) {
                $cfg['weisrc_dish']['queuemode'] = trim($_GPC['queuemode']);
                $this->saveSettings($cfg);
                // message('更新成功！', $this->createWebUrl('queuesetting2', array('op' => 'setting', 'storeid' => $storeid)), 'success');
                $url=$this->createWebUrl('queuesetting2', array('op' => 'setting', 'storeid' => $storeid));
                header("location:$url");
            }
        }
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
    ...
    //收银台排号管理
    public function doWebQueueOrder2()
    {
        global $_GPC, $_W;
//        $GLOBALS['frames'] = $this->getNaveMenu();
        $weid = $this->_weid;
        ...
        if ($operation == 'post') {
            ...
            if (!empty($id)) {
                ...
                foreach ($item['staple'] as $k => $v) {
                    $items[$k]['name'] = pdo_fetch('SELECT name FROM ci_goods WHERE id = '.$k);
                    // $item['staple'][$key]['name'] = pdo_fetch('SELECT name FROM ci_goods WHERE id = '.$k);
                    $items[$k]['id']['id'] = $k;
                    $items[$k]['weight']['wei'] = $v;
                    $items[$k]['unit'] = pdo_fetch('SELECT costName,spec FROM ci_goods WHERE id = '.$k);
                    // var_dump($item['staple'][$key]); 
            ...
            $staple = pdo_fetchall('SELECT id,name,costName,spec FROM ci_goods WHERE status=1 ORDER BY categoryId DESC');
            if (checksubmit('submit')) {
                $weight = $_GPC['weight'];
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
        ...
        if ($operation == 'display') {
            ...
            if (checksubmit('submit')) {
                ...
                if(!empty($_GPC['catetitle']) && !empty($_GPC['goodsids'])){
                        $content = implode(',', $_GPC['goodsids']).",".$_GPC['catetitle'];
                }elseif(!empty($_GPC['catetitle']) && empty($_GPC['goodsids'])){
                        $content = $_GPC['catetitle'];
                }else{
                        $content = implode(',', $_GPC['goodsids']);
                }
                ...
                message('更新套餐成功！', $this->createWebUrl('intelligent', array('op' => 'display', 'storeid' => $storeid)), 'success');
            }
            include $this->template('intelligent');
        } elseif ($operation == 'delete') {
            $id = intval($_GPC['id']);
            ...
            if (empty($category)) {
                message('抱歉，套餐不存在或是已经被删除！', $this->createWebUrl('intelligent', array('op' => 'display', 'storeid' => $storeid)), 'error');
            }
            pdo_delete($this->table_intelligent, array('id' => $id, 'weid' => $weid));
            message('套餐删除成功！', $this->createWebUrl('intelligent', array('op' => 'display', 'storeid' => $storeid)), 'success');
        }
    }
...
//        $GLOBALS['frames'] = $this->getNaveMenu();
        ...
        //昨天
        $yeststart = strtotime('yesterday'); 
        $yestend = mktime(0,0,0,date('m'),date('d'),date('Y'));
        //本月
        $month_start=mktime(0,0,0,date('m'),1,date('Y'));
        $month_end=mktime(23,59,59,date('m'),date('t'),date('Y'));

        //$last_day = strtotime(date('Y-m-d H:i:s', time() - 60*60*24*30));
            ...
            if (!empty($list)) {
                foreach ($list as $row) {
                    $userids[$row['from_user']] = $row['from_user'];
                        ...
                        //昨天销售额
            $yesterday = pdo_fetch("SELECT SUM(totalprice) as yest FROM ims_weisrc_dish_order WHERE ispay=1  AND dateline>$yeststart and dateline<=$yestend AND storeid= ".$storeid);
            //本月销售额
            $month = pdo_fetch("SELECT SUM(totalprice) as month FROM ims_weisrc_dish_order WHERE ispay=1 AND dateline>$month_start and dateline<=$month_end AND storeid= ".$storeid);
            //累计销售额
            $moneyall = pdo_fetch("SELECT SUM(totalprice) as moneyall FROM ims_weisrc_dish_order WHERE ispay=1 AND storeid= ".$storeid);

            // //销售排行
            // $goodsRank = pdo_fetchall("SELECT * FROM " . tablename($this->table_goods) . " WHERE storeid= ".$storeid);
            // foreach ($goodsRank AS $key => $value){
            //     $goodsRank[$key]['new'] = pdo_fetch("select count(price) as new from " . tablename($this->table_order_goods) . " where goodsid=".$value['id']." and dateline>=UNIX_TIMESTAMP(DATE_SUB(now(),INTERVAL 30 day))");
            //     $goodsRank[$key]['allline'] = pdo_fetch("select count(price) as allline from " . tablename($this->table_order_goods) . " where goodsid=".$value['id']);
            // }
            // $goodscount = array();
            // foreach ($goodsRank as $goods) {
            //     $goodscount[] = $goods['new'];
            // }
            // array_multisort($goodscount,SORT_DESC,$goodsRank);
