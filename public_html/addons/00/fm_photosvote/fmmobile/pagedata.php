		...
		//output results from database 
			if ($reply['tmoshi'] == 1 || $reply['tmoshi'] == 2 || $reply['tmoshi'] == 3) {
				foreach ($userlist as $mid => $row) {					
					...
					$result = $result.'<div class="toupiao" id="'.$row['id'].'" style=" padding: 0px 10px 0px;  height: 70px; text-align:center;">';
					...
					if ($reply['tpname']) {			
						$result = $result.'<a href="javascript:void(0)" id="'.$row['id'].'" class="btn  btn-danger btn-small"  data-toggle="tooltip" data-placement="top" ';										
