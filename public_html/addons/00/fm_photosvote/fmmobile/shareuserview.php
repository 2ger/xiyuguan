...
defined('IN_IA') or exit('Access Denied');
	...
	if ($cfg['oauthtype'] == 1) {
		...
		if (isset($avatar) && isset($nickname) && isset($from_user) ){
			...
			$from_user = $_W['openid'];
			if($from_user) {//$from_user 修改 !
				//取得openid后查询是否为高级号
				if ($serverapp ==4) {//认证服务号查询是否关注 //==4 修改
					...
					if($_W['fans']['follow'] || !$profile['follow']){//已关注直接获取信息 //$profile['follow'] 修改
					//   var_dump('ddd');
					// die();
						...
						if(empty($info) || !is_array($info) || empty($info['openid'])  || empty($info['nickname']) ) { //修改 
