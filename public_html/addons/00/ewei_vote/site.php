...
//var_dump($from_user);
      //  if ((substr($from_user, 0, 4) == 'we7_') && ($_GPC['share'] == 1)) {
        ...
        if ($reply['votelimit'] == 1) {
            ...
            $limits = "活动时间: " . date('Y-m-d H:i', $reply['starttime']) . " 至 " . date('Y-m-d H:i', $endtime);
        }
        $selects = "";
        if ($reply['votetype'] == 0) {
            $selects = "最多选择一项";
...
//列出所选 2016/5/22
           $sql2 = 'SELECT * FROM ' . tablename('vote_fans') . ' WHERE from_user ="'.$from_user.'" LIMIT 1';//oq5x0uAvWVZpLhW7HORqp-N_QJwU
		  
		   $list2 = pdo_fetchall($sql2);
		    $votetime= date('Y-m-d H:i', $list2[0]['votetime']);//2016/5/24
		//    var_dump($sql2);
		//  var_dump($list2);
