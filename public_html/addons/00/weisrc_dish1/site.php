...

{
    ...
    //     var_dump($from_user);
     ...
     //   $from_user = 'testuser';
        ...
        $user = pdo_fetch("SELECT * FROM " . tablename($this->table_fans) . " WHERE weid = :weid  AND username=:from_user ORDER BY `id` DESC limit 1", array(':weid' => $weid, ':from_user' => $from_user));
...
//        $order = pdo_fetch("SELECT a.* FROM " . tablename($this->table_order) . " AS a LEFT JOIN " . tablename($this->table_stores) . " AS b ON a.storeid=b.id  WHERE a.id ={$id} AND a.from_user='{$from_user}' ORDER BY a.id DESC LIMIT 20");
    ...
    //   var_dump($from_user);
        ...
        $order['goods'] = pdo_fetchall("SELECT a.*,b.* FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id WHERE a.weid = '{$weid}' and a.orderid={$order['id']}");
...
$list = pdo_fetchall("SELECT a.*,b.thumb,b.title,c.title as sortname,d.title as tablesname FROM " . tablename($this->table_order_goods) . " as a left join  " . tablename($this->table_goods) . " as b on a.goodsid=b.id left join " . tablename($this->table_stores) . " as c on a.storeid=c.id left join " . tablename($this->table_tables) . " as d on a.tablesid=d.id  WHERE a.weid = '{$weid}' and a.tablesid={$opera['tablesid']} and a.storeid={$opera['storeid']}  and a.stepnow < 5 and a.stepnow >0");  //and a.stepnow= {$opera['step']}
...
public function doMobileOperadetail()

    {
        ...
        $opera['step'] = $_GPC['stepnow']+1;

        $stepman = 'step'.$opera['step'].'man';

//按扫描次数判断操作 

$resule = pdo_query("UPDATE " . tablename($this->table_order_goods) . " SET stepnow=stepnow+1 WHERE id=:id", array(':id' => $orderid));



  // message('支付成功！', $this->createMobileUrl('opera', array('storeid' => $opera['storeid'],'tablesid' => $opera['tablesid'],'step' => $opera['step']), true), 'success');
