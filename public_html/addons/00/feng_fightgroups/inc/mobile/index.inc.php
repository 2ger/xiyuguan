...
if ($this -> module['config']['mode'] == 1) {
	...
	if ($operation == 'display') {
		...
		$psize = 10;
		$goodses = pdo_fetchall("SELECT * FROM " . tablename('tg_goods') . " WHERE uniacid = '{$_W['uniacid']}' AND isshow = 1 $condition ORDER BY displayorder desc LIMIT " . (1 - 1) * $psize . ',' . $psize);
		...
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_goods') . "WHERE uniacid = '{$_W['uniacid']}' AND isshow = 1 $condition ");
