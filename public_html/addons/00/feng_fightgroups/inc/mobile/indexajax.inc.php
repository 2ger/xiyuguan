<?php
	...
	$psize = 10;
	$list = pdo_fetchall("SELECT * FROM ".tablename('tg_goods')." WHERE uniacid = '{$_W['uniacid']}' AND isshow = 1 $condition order by displayorder desc LIMIT ".($pindex-1) * $psize.','.$psize); 
	foreach($list as$k=> $v){
