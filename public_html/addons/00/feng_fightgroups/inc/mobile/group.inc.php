<?php
		...
		$payreslut = $_GPC['payreslut'];//支付成功弹出框
	  	if(!empty($tuan_id)){
		    //取得该团所有订单
		    $orders = pdo_fetchall("SELECT * FROM " . tablename('tg_order') . " WHERE uniacid ='{$_W['uniacid']}' and tuan_id = {$tuan_id} and is_tuan in(1,2,3) and status in(1,2,3,4,6,7) ORDER BY starttime asc");
