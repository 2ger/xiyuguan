...
$this -> backlists();
load() -> func('tpl');
$ops = array('display', 'edit', 'delete', 'recycle', 'redel','return','check','set');
...
if ($op == 'check') {
	...
	$psize = 10;
	//设置分页大小
	$condition = " uniacid = '{$_W['uniacid']}' and isshow=2 ";
	...
	include $this -> template('web/confirm');
}
if($op == 'set'){
	global $_GPC, $_W;
		$id = intval($_GPC['id']);
		...
		if($type=='isshow1'){
			pdo_update("tg_goods", array('isshow' => 2), array("id" => $id, "uniacid" => $_W['uniacid']));
		}
		if($type=='isshow2'){
			pdo_update("tg_goods", array('isshow' => 3), array("id" => $id, "uniacid" => $_W['uniacid']));
		}
		if($type=='isshow3'){
			pdo_update("tg_goods", array('isshow' => 4), array("id" => $id, "uniacid" => $_W['uniacid']));
		}
...
if ($op == 'display') {
	...
	foreach ($goodses as $key => $value) {
		$goodses[$key]['category'] = pdo_fetch("SELECT * FROM " . tablename('tg_category') . " WHERE weid = '{$_W['uniacid']}' and id = '{$value['fk_typeid']}'");
		$goodses[$key]['merchant'] = pdo_fetch("SELECT * FROM " . tablename('tg_merchant') . " WHERE uniacid = '{$_W['uniacid']}' and id={$value['merchantid']} ");
	}
