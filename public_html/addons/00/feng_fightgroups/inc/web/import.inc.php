...
//	echo "<pre>";print_r($list);exit;
	...
	$total_tuan = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2 and is_tuan in(1,2,3,4) and is_hexiao=0", $paras);
	$total_single = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2 and is_tuan=0 and is_hexiao=0", $paras);
	$total_all = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2 and is_hexiao=0");
...
//	$orders = pdo_fetchall("select * from" . tablename('tg_order') . "where $condition and status=2");
	...
	foreach ($orders as $k => $vv) {
		...
		foreach ($order_same as $kk => $v) {
			...
			$mermber = $this->getfansinfo($v['openid']);
