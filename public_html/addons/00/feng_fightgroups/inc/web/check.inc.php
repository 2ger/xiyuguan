...
if ($operation == 'display') {
	...
	if($_GPC['check']=='check'){
		...
		$res=pdo_update('tg_order', array('status' => 4, 'is_hexiao'=>2,'veropenid' => 'houtai','sendtime'=>TIMESTAMP,'gettime'=>TIMESTAMP), array('id' => $id));
		if($res){
...
//		//发货提醒
	...
	if (checksubmit('cancelsend')) {
		$res=pdo_update('tg_order', array('status' => 2), array('id' => $id));
		if($res){
