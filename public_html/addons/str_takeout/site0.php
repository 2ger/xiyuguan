...
class Str_takeoutModuleSite extends WeModuleSite {
	...
	public function doMobileLogin() {
		...
		// $users = pdo_fetch('SELECT id,name FROM ci_contact WHERE name="'.$members['title'].'"');
	    ...
	    $_SESSION['lId'] = $users['locationId'];
	    if(!empty($users)){
	    	// $url = $this->createMobileUrl('orderdetails', array('id' => $id));
	    	if($status==1){
	    		header('location: ' . $this->createMobileUrl('list', array('lId' => $users['locationId'],'status' => 0,'storeid' => $storeid,'uid' => $users['id'])));
	    	}
	    	if($status==2){
	    		header('location: ' . $this->createMobileUrl('stock', array('lId' => $users['locationId'],'storeid' => $storeid,'uid' => $users['id'])));
	    	}
	    	if($status==3){
	    		header('location: ' . $this->createMobileUrl('zigou', array('lId' => $users['locationId'],'storeid' => $storeid,'uid' => $users['id'])));
	    	}
...
//商品列表
	public function doMobileList() {
		...
		$storeid = $_GPC['storeid'];
	$store = $_GPC['store'];
		$storeinfo = pdo_fetch('select * from ci_storage where name = "'.$store.'"');
		...
		//一级菜单
		$categorys = pdo_fetchall('SELECT name, id FROM ci_category WHERE status=1 and level=1 and parentId=0 and typeNumber="trade" and id NOT IN (30,31,58)');//17.2.7隐藏桌椅等类
		//二级菜单
		// $categorys = pdo_fetchall('SELECT name, id FROM ci_category WHERE status=1 and level=2 and parentId >0 and typeNumber="trade"');

		$dish = pdo_fetchall('SELECT g.* FROM ci_goods as g join ci_invoice_info as i on g.id=i.invId WHERE g.guanlianId=0 AND  g.status = 1 AND i.locationId = 2 GROUP BY invId ORDER BY id ASC');
		$cate_dish = array();
		foreach($dish as $di) {
			$cate_dish[$di['categoryId']][] = $di;
			
			foreach ($cate_dish[$di['categoryId']] as $key => $value) {
				$guan = pdo_fetchall('SELECT id from ci_goods where guanlianId='.$value['id'].'',id);
				foreach ($guan as $key => $v) {
					$guanid[] = $v['id'];
				}
				$guanid['id'] = $value['id'];
				...
				// var_dump($guanid);
				//总店库存17.1.24
				$cate_dish[$di['categoryId']][$key]['qty'] = pdo_fetch('SELECT ifnull(sum(qty),0) as qty FROM ci_invoice_info WHERE locationId = 2 AND invId IN ('.$guanId.')');
				//本店库存17.1.24
			    $cate_dish[$di['categoryId']][$key]['now'] = pdo_fetch('SELECT ifnull(sum(qty),0) as nowqty FROM ci_invoice_info WHERE invId IN ('.$guanId.') AND locationId = '.$lId);
		    }
	...
	public function doMobileZigou() {
		...
		// $categorys = pdo_fetchall('SELECT name, id FROM ci_category WHERE status=1 and level=1 and parentId=0 and typeNumber="trade" and id NOT IN (30,31,58)');//17.2.7隐藏桌椅等类
		//自购二级菜单
		$categorys = pdo_fetchall('SELECT name, id FROM ci_category WHERE status=1 and level=2  and typeNumber="trade"');
		// and parentId =58
	...
	public function doMobileStock() {
		...
		$category = pdo_fetchall('SELECT name, id FROM ci_category WHERE status=1 and typeNumber="trade" AND id NOT IN (30,31,58)');
        // var_dump($category);

		$goods = pdo_fetchall('SELECT g.id,g.name,g.number,g.spec,g.costUnitId,g.costName,g.unitName,g.categoryId,sum(i.qty) as qty FROM ci_goods as g join ci_invoice_info as i on g.id=i.invId WHERE i.locationId = '.$lId.' '.$wheres.' GROUP BY invId ORDER BY id ASC');
		// var_dump($goods);
		$cate_goods = array();
		foreach($goods as $gd) {
...
// var_dump($uid);
		...
		if(!empty($dishes)) {
			...
			if($status==3){
				$dish_info = pdo_fetchall('SELECT * FROM ci_goods WHERE id IN ('.$ids_str.') {GROUP BY id');
			}else{
...
// var_dump($uid);
		...
		if($checked == 1 ) {
			$where .= ' AND billStatus >=0 AND checked = :checked';
			// $params[':status'] = $status;
			$params[':checked'] = $checked;
		} 
		if($status == 2 ) {
			$where .= ' AND billStatus >2';
			// $params[':status'] = $status;
			// $params[':checked'] = $checked;
		}
		...
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ci_order' . $where, $params);
		$data = pdo_fetchall('SELECT * FROM ci_order' . $where . $limit, $params);
		$dates = pdo_fetchall('SELECT * FROM ci_invoice WHERE uid='.$uid.' AND transType=150501 AND billType="PUR" AND locationId='.$lId.' ORDER BY modifyTime DESC');
		// var_dump($dates);
		$pager = pagination($total, $pindex, $psize, '', array('before' => 0, 'after' => 0));
		include $this->template('myorders');
	}
...
// var_dump($uid);
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		 ...
		 //    $lId = $_GPC['lId'];
			...
			if(!empty($imageurl)){
				if($state==3){
					pdo_fetch('UPDATE ci_invoice SET photo="'.$imageurl.'" WHERE id='.$orderid);
				}else{
		...
		if($state==3){
			$order = pdo_fetch('SELECT * FROM ci_invoice WHERE uid = :uid AND id = :id', array(':uid' => $uid,':id' => $oid));
		}else{
		...
		if($state==3){
			$order_info = pdo_fetchall('SELECT ABS(ii.qty) as qty,g.name,g.unitName,g.spec FROM ci_invoice_info as ii join ci_goods as g on ii.invId=g.id WHERE ii.iid='.$order['id']);
		}else{
