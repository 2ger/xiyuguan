...
//加入购物车确认订单
function set_order_carts($uid) {
	global $_W, $_GPC;
	if(!empty($_GPC['dish'])) {
		$num = 0;
		// $price = 0;
		$ids_str = implode(',', array_keys($_GPC['dish']));
		// $ids_num = implode(',', array_values($_GPC['dish']));
		// $dish_info = pdo_fetchall('SELECT * FROM ci_goods WHERE id IN ($ids_str)','id');

		$dish_info = pdo_fetchall("SELECT * FROM ci_goods  WHERE id IN ($ids_str) and 1=:sid", array(':sid' => 1), 'id');
		// var_dump($dish_info);
		// die();
		$grant_credit = 0;
		foreach($_GPC['dish'] as $k => $v) {
			...
			if($k && $v) {
				$dishes[$k] = $v;
				$num += $v;
				$price += ($dish_info[$k]['salePrice'] * $v);//商品零售价
				$grant_credit += ($dish_info[$k]['grant_credit'] * $v); //商品进货价
			}
		}

		$isexist = pdo_fetchcolumn('SELECT id FROM ' . tablename('str_order_cart') . " WHERE uid = :uid", array(':uid' => $uid));
		$data = array(
			'uniacid' => $_W['uniacid'],
			'sid' => $sid,
			'uid' => $uid,
			'num' => $num,
			'price' => $price,
			'grant_credit' => $ids_str,//商品id
			'data' => iserializer($dishes),//序列化商品信息
			'addtime' => TIMESTAMP,
		);
		if(empty($isexist)) {
...
//获取商品信息
function get_order_carts($uid) {
	global $_W, $_GPC;
	$cart = pdo_fetch('SELECT * FROM ' . tablename('str_order_cart') . " WHERE uid = :uid", array(':uid' => $uid));
	if(empty($cart)) {
		return array('num' => 0, 'price' => 0);
	}
	if((TIMESTAMP - $cart['addtime']) > 7*86400) {
...
//重选菜品
function del_order_carts($uid) {
	global $_W;
	pdo_delete('str_order_cart', array('uid' => $uid));
	return true;
}
