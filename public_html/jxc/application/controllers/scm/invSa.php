...
class InvSa extends CI_Controller {
	...
	public function index() {
		...
		switch ($action) {
			case 'initSale':
        //商城的销量对照表 START
        $data_choose = $_GET['data'];
        if($data_choose == ""){
            $data_choose = date('Y-m-d',time());
        }
       $data['data_choose']= $data_choose; 
        $start =     strtotime($data_choose);
        $end = $start-86400;
        $Lastweeks=mktime(0,0,0,date('m'),date('d')-date('w')+1-7,date('Y'));
        $Lastweeke=mktime(23,59,59,date('m'),date('d')-date('w')+7-7,date('Y'));
        ...
        $where = ' and dateline > '.$start.' and dateline < '.$end;
        // var_dump($where);
        $data['result'] = $this->mysql_model->query('select  a.*  from ims_weisrc_dish_goods as a where 1=1 order by id',2); //所有产品
        foreach ($data['result'] as $key => $value) {
            $countnum= $this->mysql_model->query('select  count(total) as count  from ims_weisrc_dish_order_goods where goodsid='.$value['id'].' and dateline > '.$end.' and dateline < '.$start,1); //当日销量
        $data['result'][$key]['count']= $countnum;
            //var_dump($countnum);
        }

        foreach ($data['result'] as $key => $value) {
            $countweek= $this->mysql_model->query('select  count(total) as count  from ims_weisrc_dish_order_goods where goodsid='.$value['id'].' and dateline > '.$Lastweeks.' and dateline < '.$Lastweeke,1); //上周销量
        $data['result'][$key]['countweek']= $countweek;
           // var_dump($countweek);
        }
        foreach ($data['result'] as $key => $value) {
           $countmonth= $this->mysql_model->query('select  count(total) as count  from ims_weisrc_dish_order_goods where goodsid='.$value['id'].' and dateline > '.$premonth.' and dateline < '.$premonthed,1); //上个月销量
        $data['result'][$key]['countmonth']= $countmonth;
           // var_dump($countmonth);
        }
	...
	public function updateInvSa(){
		...
		if (strlen($data)>0) {
		    ...
		    $info = elements(array(
				...
				'salesId','accId','modifyTime'),$data,NULL);
			$this->db->trans_begin();
