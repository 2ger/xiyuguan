...
<div class="center">
	...
	<section>
        ...
        <article>
            ...
            <dl dUnitName="<?php  echo $goods['unitname'];?>" dSubCount="<?php  echo $goods['subcount'];?>" dishid="<?php  echo $goods['id'];?>" dName="<?php  echo $goods['title'];?>" dTaste="<?php  echo $goods['taste'];?>" dDescribe="<?php  echo $goods['description'];?>" dPrice="<?php  echo $goods['productprice'];?>" dIsHot="<?php  if($goods['subcount']>20) { ?>2<?php  } else { ?>1<?php  } ?>" dPicture="<?php  echo tomedia($goods['thumb']);?>" dSpecialPrice="<?php  echo $goods['marketprice'];?>" dIsSpecial="<?php  echo $goods['isspecial'];?>">
                <dt><h3><?php  echo $goods['title'];?></h3></dt>
                <dd><a href="javascript:void(0)" class="dataIn"><img src="<?php  echo tomedia($goods['thumb']);?>"  alt="12"  title="" /></a></dd>
                <dd>
                    <?php  if($goods['isspecial']==1) { ?>
                    <em><?php  echo $goods['productprice'];?>元/<?php  echo $goods['unitname'];?></em>
                    <?php  } else if($goods['isspecial']==3) { ?>
                    <em><?php  echo $goods['marketprice'];?>元/<?php  echo $goods['unitname'];?><!-- <del><?php  echo $goods['productprice'];?>元/<?php  echo $goods['unitname'];?></del> --></em> <!--注释原价-->
	...
	<footer class="footFix">
        <a class="btn_add" href="<?php  echo $this->createMobileUrl('waplist', array('storeid' => $storeid, 'from_user' => $from_user, 'tablesid' => $tablesid, 'mode' => $mode), true)?>" >返回列表</a>
		<a id="addToMenu" class="btn_add" href="<?php  echo $this->createMobileUrl('addtomenu', array('storeid' => $storeid, 'from_user' => $from_user, 'num' => $num, 'intelligentid' => $intelligentid, 'tablesid' => $tablesid, 'mode' => $mode), true)?>">加入菜单</a>
		<a class="btn_change" href="<?php  echo $this->createMobileUrl('wapselectlist', array('storeid' => $storeid, 'from_user' => $from_user, 'num' => $num, 'intelligentid' => $intelligentid, 'tablesid' => $tablesid, 'mode' => $mode), true)?>" onclick="showDish()">换一桌</a>
	</footer>
</div>
...
isMenuFilled = (isMenuFilled == '1');
    ...
    function popPic(imgUrl,title,price, isSpecial, specialPrice, people,teast,assess,isHot){
        ...
        if (isSpecial == view_const_dish_SPECIAL_PRICE_YES || isSpecial == view_const_dish_SPECIAL_PRICE_VIP) {
            _tmpHtml += "<i>"+specialPrice+"</i>";//<del>"+_price+"</del>注释原价
        } else {
            _tmpHtml += "<i>"+_price+"</i>";
        }
