...
<div class="container" onselectstart="return true;" ondragstart="return false;">
	<header class="nav menu">
	    <div>
		    <a class="on"><?php  echo $transTypeName;?></a>
	    </div>	
	</header>
	...
	<form name="cart_form" action="<?php  echo $this->createMobileUrl('orderlist', array('status' => $status,'lId' => $_GPC['location'],'storeid'=>$_GPC['storeid'],'uid' => $_GPC['erpuid']), true);?>" method="post">
		...
		<section class="menu_wrap skin1" id="menuWrap">
			...
			<!-- 搜索 -->
				<?php  if(is_array($categorys)) { foreach($categorys as $cate_row) { ?>
					<div class="menu_tt cat<?php  echo $cate_row['id'];?>" id="cate-<?php  echo $cate_row['id'];?>"><h2 style="color:#ff510c"><?php  echo $cate_row['name'];?></h2></div>
					<ul class="menu_list list0 cat<?php  echo $cate_row['id'];?>">
						<?php  if(is_array($cate_dish[$cate_row['id']])) { foreach($cate_dish[$cate_row['id']] as $ds) { ?>
							<li id="<?php  echo $ds['id'];?>">
								<div class="hidden"></div>
								<div>
									<h3><?php  echo $ds['name'];?></h3>
									<p>
										本店库存<span class="sale_num"><?php  echo $ds['now']['nowqty'];?></span><span class="sale_unit"><?php  echo $ds['unitName'];?></span>
									</p>
									<?php  if($ds['spec']!='') { ?>
									规格：<?php  echo $ds['spec'];?>
									<?php  } ?>
								...
								<div class="price_wrap">
									<div class="fr " max="<?php  echo $ds['qty'];?>">
										<?php  if($status ==3) { ?>
										<input autocomplete="off" class="inputnum h_num" id="inputnum"   type="number" name="price[<?php  echo $ds['id'];?>]" value="<?php  echo $cart['data'][$ds['id']];?>" placeholder="价格(元)"> 
										<?php  } ?>
											<input autocomplete="off" class="inputnum h_num" id="inputnum"   type="number" name="dish[<?php  echo $ds['id'];?>]" value="<?php  echo $cart['data'][$ds['id']];?>" placeholder="数量">
											<br style="clear:both">
