...
<ul class="nav nav-tabs">
	<li class="active"><a href="<?php  echo url('platform/menu/display');?>">自定义菜单</a></li>
</ul>
<?php  if($do == 'display') { ?>
<div class="clearfix">
		...
		<div class="panel-body table-responsive">
			<table class="table table-hover">
				...
				<tr>
					...
					<td>
						<?php  if($da['type'] == 3) { ?>
							<?php  if($da['sex'] > 0) { ?>性别:<?php  echo $names['sex'][$da['sex']];?> <br><?php  } ?>
							<?php  if($da['group_id'] != -1) { ?>粉丝分组:<?php  if($da['group_id'] == -1) { ?>不限<?php  } else { ?><?php  echo $groups[$da['group_id']]['name'];?><?php  } ?><br><?php  } ?>
							<?php  if($da['client_platform_type'] > 0) { ?>客户端:<?php  echo $names['client_platform_type'][$da['client_platform_type']];?> <br><?php  } ?>
							<?php  if(!empty($da['area'])) { ?>地区:<?php  echo $da['area'];?><?php  } ?>
						<?php  } else { ?>
							所有粉丝
						<?php  } ?>
					</td>
					<td><?php  if($da['createtime']) { ?><?php  echo date('Y-m-d H:i', $da['createtime']);?><?php  } else { ?>未知<?php  } ?></td>
					<td>
						<?php  if($da['status'] == 1) { ?>
					...
					<td align="right">
						...
						<?php  } ?>
						<a href="<?php  echo url('platform/menu/remove', array('id' => $da['id']));?>" onclick="if(!confirm('<?php  if($da['type'] == 1) { ?>删除默认菜单会清空所有菜单记录，确定吗？<?php  } else { ?>确定删除菜单?<?php  } ?>')) return false;" class="btn btn-default">删除</a>
					</td>
				</tr>
				<?php  } } ?>
			</table>
...
<div class="conditionMenu" ng-controller="conditionMenuDesigner" id="conditionMenuDesigner">
	...
	<?php  if($menu['type'] == 1 || (!$id) || !$menu['status']) { ?>
...
require(['angular', 'underscore', 'jquery.ui', 'jquery.caret', 'wechatDistrict'], function(angular, _, $, $, dis){
	...
	angular.module('app', []).controller('conditionMenuDesigner', function($scope, $http){
		...
		<?php  if($id > 0 && $type != 1 && $params['status'] == 1) { ?>
