    ...
    <form action="" method="post" class="form-horizontal form" enctype="multipart/form-data">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <!-- 原材料用量 -->
                <div class="form-group">
                    <label class="col-xs-12 col-sm-3 col-md-2 control-label">原材料用量</label>
                    <div class="col-sm-9 col-xs-12 form-inline">
                        <div class="help-block">
                            <select id="selectyuan" name="staple" class="form-control">
                                <option value="0">请选择</option>
                                <?php  if(is_array($staple)) { foreach($staple as $row) { ?>
                                <option value="<?php  echo $row['id'];?>" unit="<?php  echo $row['costName'];?>" spec="<?php  echo $row['spec'];?>"><?php  echo $row['name'];?></option>
                                <?php  } } ?>
                            </select>
                            <a class="btn btn-success" id="addyuan">添加</a>

                            <input type="text" class="form-control" id="key"> 
                            <a class="btn btn-success" id="keysearch">搜索</a>

                            <script type="text/javascript">
                             $('#keysearch').click(function (){
                                    var key = $('#key').val();
                                    $("option:contains("+key+")").attr("selected",true);
                                  
                                });
                                $('#addyuan').click(function (){
                                    var name = $('#selectyuan').find("option:selected").text();
                                    var yuanid = $('#selectyuan').val();
                                    var unit = $('#selectyuan').find("option:selected").attr('unit');
                                    var spec = $('#selectyuan').find("option:selected").attr('spec');//规格

                                    var html="<tr id = 'l"+yuanid+"'><td>"+name+"</td><td><input type=text name='weight["+yuanid+"]' class=form-control style='width: 60px;'></td><td>"+unit+"</td><td>"+spec+"</td><td><input type='button' class='btn btn-default delyuan' id = "+yuanid+" value='删除'></td></tr>";
                                    $('#yuan').append(html);
                                    delBtnEvent(yuanid);
                                });
                                function delBtnEvent(id){
                                    $('.delyuan').bind("click",function(){
                                        // ('#'+id).remove();
                                        $("tr").remove('#l'+id);
                                        // alert(id);
                                    });
                            ...
                            <table class="table table-hover table-bordered" style="width:400px;margin-top: 10px;">
                            <tr>
                                <th>原材料</th>
                                <th>数量</th>
                                <th>单位</th>
                            ...
                            <tbody id="yuan">
                            <?php  if(is_array($items)) { foreach($items as $rows) { ?>
                            <tr  id = "<?php  echo $rows['id']['id'];?>">
                                <td><?php  echo $rows['name']['name'];?></td>
                                <td><input type="text" class=form-control style='width: 60px;' name="weight['<?php  echo $rows['id']['id'];?>']" value="<?php  echo $rows['weight']['wei'];?>"></td>
                                <td><?php  echo $rows['unit']['costName'];?></td>
                                <td><?php  echo $rows['unit']['spec'];?></td>
                                <td><a class="btn btn-default delyuan"  cid = "<?php  echo $rows['id']['id'];?>">删除</a></td>
                            </tr>
                            <?php  } } ?>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        ...
                        <label class="checkbox-inline">
                            ...
                            <?php  } } ?>
                            <input type="hidden" class="tasteid" name="tasteid" value="">
                            <label class="checkbox-inline"><a href="/web/index.php?c=site&a=entry&op=post&do=taste&storeid=<?php  echo $storeid;?>&m=weisrc_dish">添加做法</a></label>
                            <script>
$('.tastecheck').click(function(){
var checked = $("input:checkbox[name='message']:checked").map(function(index,elem) {
            return $(elem).val();
        }).get().join(',');
    //alert(checked);
    $('.tasteid').val(checked);
