...
<ul class="nav nav-tabs">
	<li<?php  if($do == 'index') { ?> class="active"<?php  } ?>><a href="<?php  echo url('activity/clerk/index');?>">功能说明</a></li>
...
<div class="panel panel-default">
	<div class="panel-body">
		<p>1. 管理员添加 <a href="<?php  echo url('activity/clerk/list');?>">店员及消费密码</a>；</p>
			...
			<div class="panel-body">
				<div class="form-group">
					...
					<div class="col-sm-9 col-xs-12">
						<input type="text" name="name" value="<?php  echo $clerk['name'];?>" class="form-control" placeholder="请填写店员姓名">
					</div>
				</div>
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label"><span class="require">*</span>登陆账号</label>
					<div class="col-sm-9 col-xs-12">
						<input type="text" name="username" value="<?php  echo $clerk['username'];?>" class="form-control">
				...
				<table class="table">
					<?php  if(is_array($permission)) { foreach($permission as $name => $row) { ?>
						...
						<?php  if(is_array($row['items'])) { foreach($row['items'] as $item) { ?>
							...
							<td>
								<div class="checkbox">
									<label class="permission-child permission-child-<?php  echo $name;?>" data-name="<?php  echo $name;?>"><input type="checkbox" value="<?php  echo $item['permission'];?>" <?php  if(in_array($item['permission'], $clerk['permission'])) { ?>checked<?php  } ?> name="permission[]"><?php  echo $item['title'];?></label>
								</div>
							</td>
							<?php  if($i%6 == 0) { ?></tr><?php  } ?>
							<?php  $i++;?>
