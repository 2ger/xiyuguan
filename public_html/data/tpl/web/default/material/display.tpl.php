		...
		<?php  if(is_array($lists)) { foreach($lists as $list) { ?>
			<div class="col-md-3">
				<div class="panel panel-default panel-image">
					<div class="panel-body">
						<div class="content">
							<img src="<?php  echo url('utility/wxcode/image', array('attach' => tomedia($list['attachment'], true)));?>" width="100%" height="160" alt="<?php  echo $list['filename'];?>">
							<span><?php  echo $list['filename'];?></span>
						</div>
						<div class="btn-group">
							<a href="javascript:;" class="btn btn-default btn-sm btn-send" data-id="<?php  echo $list['id'];?>" data-type="<?php  echo $list['type'];?>">群发</a>
							<a href="javascript:;" class="btn btn-default btn-sm btn-view" data-id="<?php  echo $list['id'];?>" data-type="<?php  echo $list['type'];?>">预览</a>
							<a href="javascript:;" class="btn btn-default btn-sm btn-del" data-id="<?php  echo $list['id'];?>" data-type="<?php  echo $list['type'];?>">删除</a>
						</div>
					</div>
				</div>
			</div>
					...
					<div class="panel-body">
						<div class="audio-msg">
							...
							<div class="audio-content">
								<div class="audio-title"><?php  echo $list['filename'];?>dklaklakla看见了的角度看都看绝对空间都看得见看得见看到绝对看得见肯定健康的健康的就</div>
				...
				<div class="panel panel-default panel-video">
					<div class="panel-body">
						<div class="video-content">
							<h4 class="title text-muted"><?php  echo $list['tag']['title'];?></h4>
							<div class="date text-muted"><?php  echo date('m月d日 H:i', $list['createtime']);?></div>
							<div class="video">
								<img src="../web/resource/images/banner-bg.png" alt="" />
							</div>
							<div class="abstract text-muted" style="overflow:hidden"><?php  echo $list['tag']['description'];?></div>
						</div>
						<div class="btn-group">
							<a href="<?php  echo $list['tag']['down_url'];?>" target="_blank" class="btn btn-default btn-sm">下载</a>
							<a href="javascript:;" class="btn btn-default btn-sm btn-send" data-id="<?php  echo $list['id'];?>" data-type="<?php  echo $list['type'];?>">群发</a>
							<a href="javascript:;" class="btn btn-default btn-sm btn-view" data-id="<?php  echo $list['id'];?>" data-type="<?php  echo $list['type'];?>">预览</a>
							<a href="javascript:;" class="btn btn-default btn-sm btn-del" data-id="<?php  echo $list['id'];?>" data-type="<?php  echo $list['type'];?>">删除</a>
						</div>
			...
			<?php  if(is_array($lists)) { foreach($lists as $list) { ?>
				<div class="col-xs-12 col-sm-12 col-md-6 col-lg-3 water">
					<div class="panel-group">
						<?php  if(is_array($list['items'])) { foreach($list['items'] as $key => $item) { ?>
						<?php  if(!$key) { ?>
						<div class="panel panel-default">
							<div class="panel-body">
								<div class="img">
									<i class="default"></i>
									<a href="<?php  echo $item['url']?>" target="_blank"><img src="<?php  echo url('utility/wxcode/image', array('attach' => $item['thumb_url']))?>"></a>
						...
						<div class="panel panel-default">
							<div class="panel-body">
								<a href="<?php  echo $item['url']?>" target="_blank">
									...
									<div class="img">
										<img src="<?php  echo url('utility/wxcode/image', array('attach' => $item['thumb_url']))?>">
						...
						<div class="col-sm-9 col-xs-12">
							<select name="group" id="group" class="form-control">
								...
								<?php  if(!empty($groups)) { ?>
									...
									<option value="<?php  echo $group['id'];?>"><?php  echo $group['name'];?>【<?php  echo $group['count'];?>人】</option>
									<?php  } } ?>
								<?php  } ?>
							</select>
	...
	require(['angular', 'wechatFileUploader', 'jquery.jplayer', 'jquery.wookmark'], function(angular, uploader){
		...
		$('.btn-view').click(function(){
			...
			$('#modal-view .btn-view').unbind().click(function(){
				...
				$.post("<?php  echo url('material/display/purview/');?>", {id:id, wxname:wxname, type:type}, function(data){
					if(data != 'success') {
						util.message(data, '', 'error');
					} else {
						util.message('发送成功', '', 'success');
