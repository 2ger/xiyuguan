// console.log(Number(parent.SYSTEM.pricePlaces));
var ispi = 0;
	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			var f = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				g = '<a id="add" class="ui-btn ui-btn-sp">新增</a> <a id="reload" class="ui-btn ui-btn-sp">刷新</a>  ',
                // <a id="ling" class="ui-btn ui-btn-sp">修改为配送价</a>   <a id="edit" class="ui-btn">保存</a><a href="../scm/invSo/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>  
                g=g+' <a href="../scm/invSo/toPdf?action=toPdf&id=' + a.id + '&ispi=1" target="_blank" class="ui-btn">成本价打印</a>';
                g=g+' <a href="../scm/invSo/toPdf?action=toPdf&id=' + a.id + '&ispi=2" target="_blank" class="ui-btn">配送价打印</a>';
                g=g+'<a id="ling" class="ui-btn ui-btn-sp">修改为配送价</a> <a id="pifa" class="ui-btn ui-btn-sp">修改为成本价</a>';
                g=g+'<a id="edit" class="ui-btn">保存</a>';
// http://yhs.xyj0772.com/erp/index.php/scm/invSo/toPdf?action=toPdf&id=55&ispi=1
				h = '<a id="add" class="ui-btn ui-btn-sp">新增</a> ',
                    //<a id="ling" class="ui-btn ui-btn-sp">修改为配送价</a> <a id="pifa" class="ui-btn ui-btn-sp">修改为成本价</a>  <a href="../scm/invSo/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a> 
                h=h+' <a href="../scm/invSo/toPdf?action=toPdf&id=' + a.id + '&ispi=1" target="_blank" class="ui-btn">成本价打印</a>';
                h=h+' <a href="../scm/invSo/toPdf?action=toPdf&id=' + a.id + '&ispi=2" target="_blank" class="ui-btn">配送价打印</a>';
                h=h+'<a id="ling" class="ui-btn ui-btn-sp">修改为配送价</a><a id="pifa" class="ui-btn ui-btn-sp">修改为成本价</a>';
                h=h+'<a id="save" class="ui-btn">保存</a>';
                i = "",
				j = "",
		...
		loadGrid: function(a) {
			...
			q.newId = 9;
            if(parent.SYSTEM.userName != "admin"){
                console.log("非管理员 hiddenAmount");
                var isHidden = "hiddenAmount";
            }else{
                console.log("是管理员 admin");
                var isHidden = "hiddenAmount";
                //hiddenAmount
            }
			var t = [{
				...
				afterEditCell: function(a, b, c, d, e) {
					function f() {
						...
						if (b) {
                            ...
                            console.log("561");
							b = $.extend(!0, {}, b), b.skuName = c.skuName, b.mainUnit = c.mainUnit, b.unitId = c.unitId, b.qty = c.qty,b.qty0 = c.qty, b.price = c.price, b.discountRate = c.discountRate, b.deduction = c.deduction, b.amount = c.amount, b.taxRate = c.taxRate, b.tax = c.tax, b.taxAmount = c.taxAmount, b.locationName = c.locationName, $("#" + a).data("goodsInfo", b)
						}
					}
			...
			this.goodsCombo = Business.billGoodsCombo($(".goodsAuto"), {
				...
				data: function() {
					...
					if (d.prices.levelPrice) {
						var f = "";
                        // a.cLevel =2;
						a.cLevel <3  ? f = ["配送", "成本价", "VIP"][a.cLevel] + "价：" + d.prices.levelPrice : d.prices.discountRate && (f = ["折扣一", "折扣二"][a.cLevel - 3] + "价：" + d.prices.levelPrice * d.prices.discountRate / 100), f && e.push({
						// a.cLevel < 3 ? f = ["配送", "成本价", "VIP"][a.cLevel] + "价：" + d.prices.levelPrice : d.prices.discountRate && (f = ["折扣一", "折扣二"][a.cLevel - 3] + "价：" + d.prices.levelPrice * d.prices.discountRate / 100), f && e.push({
							name: f,
							id: 1
						})
                    ...
                    e.push({
                        // a.cLevel < 3 ? f = ["配送", "成本价", "VIP"][a.cLevel] + "价：" + d.prices.levelPrice : d.prices.discountRate && (f = ["折扣一", "折扣二"][a.cLevel - 3] + "价：" + d.prices.levelPrice * d.prices.discountRate / 100), f && e.push({
                        name: "成本价："+d.prices.wholesalePrice,
                        id: 3
                    })
										...
										f[d.id] = a, Public.ajaxPost("../basedata/inventory/listBySelected?action=listBySelected", {
											...
											if (200 === c.status && c.data && c.data.result) {
												for (var d = c.data.result, e = 0, f = d.length; f > e; e++) {
													var g = d[e];
                                                    // console.log(g);
                                                    g.nearPrice && (a.prices = {}, a.prices.nearPrice = g.nearPrice), g.salePrice && (a.prices = a.prices || {}, a.prices.wholesalePrice = g.wholesalePrice,a.prices.levelPrice = g.salePrice, a.prices.discountRate = g.discountRate)
		...
		addEvent: function() {
			...
			}), $(".wrapper").on("click", "#reload", function(a) {//刷新
                window.location.reload();
			}), $(".wrapper").on("click", "#pifa", function(a) {//修改为成本价
				Public.ajaxGet("../scm/invSo/updatePrice?action=update", {
					id:originalData.id,type:1 
				}, function(a) {
                    ispi = 1;
                    console.log(a);
                    console.log(ispi);
                    $('#save').click();
                    THISPAGE.reloadData(a.data), parent.Public.tips({
							content: "一键修改为成本价成功！注意保存！"
						})
				});
			}), $(".wrapper").on("click", "#ling", function(a) {//修改为配送价
				Public.ajaxGet("../scm/invSo/updatePrice?action=update", {
					id:originalData.id,type:0 
				}, function(a) {
                    console.log(a);
                    $('#save').click();
                    THISPAGE.reloadData(a.data), parent.Public.tips({
							content: "一键修改为配送价成功！注意保存！"
						})
				});
