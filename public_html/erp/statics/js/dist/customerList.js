$(function() {
		...
		h.gridReg("grid", c), c = h.conf.grids.grid.colModel, $("#grid").jqGrid({
			...
			loadComplete: function(a) {
				if (a && 200 == a.status) {
					...
					var e = 250 === a.status ? f ? "没有满足条件的结果哦！" : "没有门店数据哦！" : a.msg;
					parent.Public.tips({
						type: 2,
						content: e
					})
	...
	function c() {
		$_matchCon = $("#matchCon"), $_matchCon.placeholder(), $("#search").on("click", function(a) {
			a.preventDefault();
			var b = "输入门店编号/ 名称/ 联系人/ 电话查询" === $_matchCon.val() ? "" : $.trim($_matchCon.val()),
				c = e ? e.getValue() : -1;
	...
	var e, f = !1,
		g = {
			...
			del: function(a) {
				$.dialog.confirm("删除的门店将不能恢复，请确认是否删除？", function() {
