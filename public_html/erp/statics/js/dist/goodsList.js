$(function() {
			...
			l = [{
				...
				formatter: function(a, b, c) {
                    ...
                    console.log(c);
                        var addGuanlian = "";
                    if(c.remark ==""){
                        addGuanlian = '<span class="ui-icon ui-icon-plus" title="添加关联"></span>';
                    }
					var d = '<div class="operating" data-id="' + c.id + '"><span class="ui-icon ui-icon-pencil" title="修改"></span><span class="ui-icon ui-icon-trash" title="删除"></span><span class="ui-icon ui-icon-pic" title="商品图片"></span>'+addGuanlian+'</div>';
					return d
				},
				title: !1
			}, {
	...
	function c() {
		...
		}),$("#grid").on("click", ".operating .ui-icon-plus", function(a) {
	...
	var d = (parent.SYSTEM, Number(parent.SYSTEM.qtyPlaces), Number(parent.SYSTEM.pricePlaces)),
		...
		g = 270,
		h = {
			operate: function(a, b , guanlian) {//a方法，b商品ID
                console.log("guanlian:"+guanlian);
                if (1 != guanlian) var title ="修改商品";
                else var title = "关联商品";
