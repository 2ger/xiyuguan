function init() {
	...
	SYSTEM.ISSERNUM || SYSTEM.enableAssistingProp || SYSTEM.ISWARRANTY || $("#tab").find(".tabItem:eq(2)").hide(), void 0 !== cRowId ? Public.ajaxPost("../basedata/inventory/query?action=query", {
		...
		200 === a.status ? (rowData = a.data, initField(), initSkuGrid(rowData.invSkus), initGrid(rowData.propertys), initCombo(), initEvent()) : parent.parent.Public.tips({
			type: 1,
			content: a.msg
		})
	}) : (initField(), initSkuGrid(), initGrid(), initCombo(), initEvent())
}
function initPopBtns() {
	var a = "add" == oper ? ["保存", "关闭"] : ["确定", "取消"];
    if (1 ==  guanlian) a = ["添加关联商品", "关闭"];
	api.button({
		id: "confirm",
		name: a[0],
		focus: !0,
...
function postData() {
	var a = "add" == oper ? "新增商品" : "修改商品",
		b = getCustomerData();
    if ("add" == oper || 1 == guanlian) {
        var operaDo = "add";
    } else{
        var operaDo = "update";
    }
    console.log(operaDo);
    if (1 ==  guanlian) a = "添加关联商品";
	if (b) {
		var c = {};
		...
		}), Public.ajaxPost("../basedata/inventory/"+operaDo+"?" , b, function(d) {//("add" == oper ? "add" : "update")
			if (200 == d.status) {
...
function getCustomerData() {
	...
	if (a) {
		var b = {
			id: rowData.id,
			guanlianId: rowData.id,
			number: $.trim($("#number").val()),
			name: $.trim($("#name").val()),
			costName: $.trim($("#costName").val()),//销售单位名称
			costUnitId: Public.currencyToNum($("#costUnitId").val()),//ID
			costQauntity: Public.currencyToNum($("#costQauntity").val()),//数量
...
function initField() {
    ...
    var d = new Date();
    var guanlianTime = d.getFullYear()+"-"+(d.getMonth()+1)+"-"+d.getDate();
    //如果是关联商品
    if(guanlian ==1) rowData.remark = rowData.remark+"关联商品："+rowData.number+rowData.name,rowData.number = "G-"+rowData.number+"-"+d.getDate(),rowData.name=rowData.name+" - "+guanlianTime;
	$("#note").placeholder(), "edit" == oper ? ($("#costName").val(rowData.costName),$("#costUnitId").val(rowData.costUnitId),$("#costQauntity").val(rowData.costQauntity),$("#number").val(rowData.number),
    $("#name").val(rowData.name),//设置名称 关联商品 加上日期
                                                $category.data("defItem", rowData.categoryId), $("#specs").val(rowData.spec), $("#storage").data("defItem", rowData.locationId),$("#unit").data("defItem", ["id", rowData.baseUnitId]), $("#unit").data("defItem", ["id", rowData.baseUnitId]), void 0 != rowData.purPrice && $("#purchasePrice").val(Public.numToCurrency(rowData.purPrice, pricePlaces)), void 0 != rowData.salePrice && ($("#salePrice").val(Public.numToCurrency(rowData.salePrice, pricePlaces)), $("#wholesalePrice").val(Public.numToCurrency(rowData.wholesalePrice, pricePlaces)), $("#vipPrice").val(Public.numToCurrency(rowData.vipPrice, pricePlaces)), $("#discountRate1").val(rowData.discountRate1), $("#discountRate2").val(rowData.discountRate2)), $("#minInventory").val(rowData.lowQty), $("#maxInventory").val(rowData.highQty), $("#note").val(rowData.remark), 
                                                    $("#barCode").val(rowData.barCode), $("#length").val(rowData.length), $("#width").val(rowData.width), $("#height").val(rowData.height), $("#weight").val(rowData.weight), rowData.isSerNum && ($isSerNum[0].checked = !0), rowData.isWarranty && ($isWarranty[0].checked = !0, $(".isWarrantyIn").show(), $safeDays.val(rowData.safeDays), $advanceDay.val(rowData.advanceDay))) : $("#storage").data("defItem", 0), api.opener.parent.SYSTEM.isAdmin || (rights.AMOUNT_INAMOUNT || $("#purchasePrice").closest("li").hide(), rights.AMOUNT_OUTAMOUNT || ($("#salePrice").closest("li").hide(), $("#wholesalePrice").closest("li").hide(), $("#vipPrice").closest("li").hide(), $("#discountRate1").closest("li").hide(), $("#discountRate2").closest("li").hide())), SYSTEM.enableStorage && $("#jdInfo").show(), SYSTEM.enableAssistingProp && ($(".prop-wrap").show().on("click", "input", function(a) {
		var b = $(this);
...
function initCombo() {
	storageCombo = $("#storage").combo({
		...
		extraListHtml: '<a href="javascript:void(0);" id="quickAddSkuProp" class="quick-add-link"><i class="ui-icon-add"></i>新增一个属性</a>',
		text: "name",
		value: "id",
		defaultSelected: -1,
		editable: !0,
...
function initEvent() {
	...
	categoryTree = Public.categoryTree($category, b), $("#specs").blur(function() {
		...
		null !== curRow && null !== curCol && (!$(a.target).closest("#grid").length > 0 && !$(a.target).closest(".pika-single").length > 0 && $grid.jqGrid("saveCell", curRow, curCol), !$(a.target).closest("#gridWarning").length > 0 && $gridWarning.jqGrid("saveCell", curRow, curCol), !$(a.target).closest("#gridSku").length > 0 && $gridSku.jqGrid("saveCell", curRow, curCol))
	...
	}), initValidator(), bindEventForEnterKey(), $("#grid").on("click", ".ui-icon-plus", function(a) {
		cancleGridEdit();
...
function cancleGridEdit() {
	null !== curRow && null !== curCol && ($grid.jqGrid("saveCell", curRow, curCol), $gridWarning.jqGrid("saveCell", curRow, curCol), $gridSku.jqGrid("saveCell", curRow, curCol), curRow = null, curCol = null)
}
...
var curRow, curCol, curArrears, api = frameElement.api,
	oper = api.data.oper,
	guanlian = api.data.guanlian,//获得参数， 是否关联
