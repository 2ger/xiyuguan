...
function BBSPop() {
    var a = $("#yswb-tab"),
        b = ['<ul id="yswbPop">', '<li class="yswbPop-title">请选择您要进入的论坛</li>', '<li><strong>产品服务论坛</strong><p>在线会计，在线进销存操作问题咨询</p></li>', '<li><strong>会计交流论坛</strong><p>会计实操，会计学习，会计资讯</p></li>', "</ul>"].join("");
...
var dataReflush, list = {
    ...
    salesOrder2: {
        name: "门店申请",
		href: "../scm/invSo?action=initSoList",
		dataRight: "SO_ADD",
		target: "sales",
...
$("#nav").on("click", "[rel=pageTab]", function(a) {
	...
	if ($("#scollUp").click(function() {
		...
		a.preventDefault(), $.dialog({
			...
			height: 180,
			title: "系统提示",
			content: '<div class="re-initialize"><h3>重新初始化系统将会清空你录入的所有数据，请慎重！</h3><ul><li>系统将删除您新增的所有商品、供应商、分店/门店</li><li>系统将删除您录入的所有单据</li><li>系统将删除您录入的所有初始化数据</li></ul><p><input type="checkbox" id="understand" /><label for="understand">我已清楚了解将产生的后果</label></p><p class="check-confirm">（请先确认并勾选“我已清楚了解将产生的后果”）</p></div>',
			icon: "alert.gif",
			okVal: "重新初始化",
			ok: function() {
		...
		var c = [{
			...
			focus: !0,
			callback: function() {
				window.open("http://wpa.qq.com/msgrd?V=3&uin=276601115&Site=莫深辰&Menu=yes&zh-CHS&accIds=" + SYSTEM.DBID)
			}
		}, {
			name: "下次再说"
		}],
