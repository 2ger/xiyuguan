var queryConditions = {
	...
	THISPAGE = {
		...
		loadGrid: function() {
			...
			function b(a, b, c) {
				return 150601 === a ? "订货" : (d.markRow.push(c.id), "退货")
			}
...
//			}, 
			...
			this.mod_PageConfig.gridReg("grid", e), e = this.mod_PageConfig.conf.grids.grid.colModel, $("#grid").jqGrid({
				url: "../scm/invSo?action=list&default=1",
		...
		addEvent: function() {
			...
			$("#search").click(function() {
				queryConditions.salesId = null, queryConditions.matchCon = "请输入单据号或门店名或备注" === a.$_matchCon.val() ? "" : $.trim(a.$_matchCon.val()), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), THISPAGE.reloadData(queryConditions)
