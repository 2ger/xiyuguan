var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			var f = '<a id="back" class="ui-btn ui-btn-sp" onclick="window.history.back();">返回</a>',
                g = "",
                h = "",
			// var f = '<!--<a id="original" class="ui-btn ui-btn-sc">选择源单</a>--><a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				// g = '<!--<a id="original" class="ui-btn ui-btn-sc">选择源单</a>--><a id="add" class="ui-btn ui-btn-sp">新增</a>  <a id="edit" class="ui-btn">保存</a>',
                //<a href="../scm/invSa/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>
// <a href="../scm/invSa/toPdf?action=toPdf&id=' + a.id + '&ispi=1" target="_blank" class="ui-btn">批发价打印</a>
...
// <a href="../scm/invSa/toPdf?action=toPdf&id=' + a.id + '&ispi=1" target="_blank" class="ui-btn">批发价打印</a>
		...
		loadGrid: function(a) {
			...
			var B = [{
				...
				classes: "goods",
				formatter: c,
				editable: !1,// 0 为可修改 1为不可修改 。 销货单不可修改
			...
			this.goodsCombo = Business.billGoodsCombo($(".goodsAuto"), {
				...
				data: function() {
					...
					if (d.prices.levelPrice) {
						var f = "";
						a.cLevel < 3 ? f = ["配送", "成本", "VIP"][a.cLevel] + "价：" + d.prices.levelPrice : d.prices.discountRate && (f = ["折扣一", "折扣二"][a.cLevel - 3] + "价：" + d.prices.levelPrice * d.prices.discountRate / 100), f && e.push({
							name: f,
							id: 1
						})
					}
