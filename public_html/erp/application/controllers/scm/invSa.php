...
class InvSa extends CI_Controller {
	...
	public function saleList(){
		...
		$list = $this->data_model->get_invoice($where.' and a.totalAmount > 0 order by '.$order.' limit '.$rows*($page-1).','.$rows);  
    ...
    public function delete() {
		...
		if (count($data)>0) {
			...
			$this->mysql_model->update('invoice_info',array('isDelete'=>1),array('iid'=>"-".$id));   //删除门店进货信息  
			if ($data['accId']>0) { 
	...
	private function invoice_info($iid,$data) {
		 $locationId = $this->data_model->get_location($iid);
		foreach ($data['entries'] as $arr=>$row) {
			$v[$arr]['iid']           = $iid;
			$v[$arr]['uid']           = $data['uid'];
			...
			$s[$arr]['uid']             = $data['uid'];
			$s[$arr]['buId']            = 12;
			$s[$arr]['billNo']          = $data['billNo'];
			$s[$arr]['billDate']        = $data['billDate'];
			$s[$arr]['invId']           = intval($row['invId']);
			$s[$arr]['skuId']           = intval($row['skuId']);
			$s[$arr]['unitId']          = intval($row['unitId']);
			$s[$arr]['billType']        = 'PUR';
			$s[$arr]['description']     = $row['description'];  
			$s[$arr]['transTypeName']   = $data['transType']==150601 ? '购货' : '退货';
            $s[$arr]['transType']       = $data['transType']==150601 ? '150501' : '150502';
			$s[$arr]['locationId']      = intval($locationId[0]['id']);
			$s[$arr]['qty']             = $data['transType']==150601 ? abs($row['qty']) : -abs($row['qty']); 
			$s[$arr]['price']           = abs($row['price']);
			$s[$arr]['amount']          = $data['transType']==150601 ? abs($row['amount']) :-abs($row['amount']);
			$s[$arr]['entryId']         = 1;
		...
		if (isset($v)) {
            ...
            //再插入新的
            if (isset($s) && ($locationId[0]['transType'] == 150601 || $locationId[0]['transType'] == 150602)) {
                $this->mysql_model->insert('invoice_info',$v);//生成门店进货单
