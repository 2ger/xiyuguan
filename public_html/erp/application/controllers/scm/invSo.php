...
// die();
		...
		// $data['result'] = $this->mysql_model->query('select  a.*  from ims_weisrc_dish_goods as a where 1=1',2); //所有产品
		// foreach ($data['result'] as $key => $value) {
		// 	$countnum= $this->mysql_model->query('select  count(total) as count  from ims_weisrc_dish_order_goods where goodsid='.$value['id'],1); //销量
		// 	$data['result'][$key]['count']= $countnum;
		// 	//var_dump($countnum);
		// }
	...
	public function soList(){
		...
		$where .= $this->common_model->get_admin_purview();
		$offset = $rows * ($page-1);
		// and a.totalAmount > 0 只列出订货，自动销货的不列出 and a.totalAmount > 0
		...
		if (strlen($data)>0) {
		  ...
		  //   $info = elements(array(
		    ...
		    $info = elements(array(
				...
				'billDate','description','totalQty','amount','rpAmount','totalAmount',
				'hxStateCode','totalArrears','disRate','disAmount','postData',
				'salesId','userName','accId','deliveryDate','modifyTime'),$data);//uid不进行修改
			$this->db->trans_begin();
	...
	// 修改为1批发价，0零售价
	public function updatePrice($type) {
	    $this->common_model->checkpurview();
        $id   = intval($this->input->get_post('id',TRUE));
        ...
        if (count($data)>0) {
            ...
            foreach ($list as $arr=>$row) {
                ...
                $v[$arr]['locationName'] = $row['locationName'];
                $v[$arr]['taxAmount']    = (float)$row['taxAmount'];
                $v[$arr]['invId']        = intval($row['invId']);//商品ID
                // $v[$arr]['price']        = (float)$row['price'];

                $goodinfo = $this->data_model->get_goods(' a.id ='.intval($row['invId']));
                if($type){
                    // $v[$arr]['price']        = 1;//批发价
                    if($goodinfo[0]['wholesalePrice']>0){
                        $v[$arr]['price']        = $goodinfo[0]['wholesalePrice'];//批发价
                    }else{
                        $v[$arr]['price']        = 1;//批发价
                    }
                }else{
                    // $v[$arr]['price']        = 10;
                    $v[$arr]['price']        = $goodinfo[0]['salePrice'];//零售价
                }
                // die(json_encode($goodinfo));

                $v[$arr]['amount']       = $v[$arr]['price']*$v[$arr]['qty'];
                $totalAmount +=  $v[$arr]['amount'];
            ...
            $info['data']['totalAmount']        = (float)abs($totalAmount); 
            $info['data']['entries']     = isset($v) ? $v : array();
            $info['data']['accId']       = (float)$data['accId'];
    ...
    public function toPdf() {
		...
		if (count($data)>0) { 
			...
			foreach ($postData['entries'] as $arr=>$row) {
                ...
                // var_dump($goodinfo);
                if($ispi==1){
                    // var_dump($goodinfo[0]['wholesalePrice']);//批发价
                    $v[$arr]['price']           = $goodinfo[0]['wholesalePrice'];
                    $v[$arr]['spec']           = $goodinfo[0]['spec'];
                    $v[$arr]['amount']          = $goodinfo[0]['wholesalePrice']*(float)abs($row['qty']);
                    $totalAmount = $totalAmount + $v[$arr]['amount'];
                }else{
				...
				$v[$arr]['locationName']    = $row['locationName'];
				$v[$arr]['description']    = $row['description'];
				// $v[$arr]['specs']    = "sss";//$row['discription'];
			}  
            if($ispi ==1){//按批发价打印
                    $data['type']  = 1;
			...
			if ($system['requiredCheckStore']==1) {  
				 if (intval($data['transType'])==150601) {                        //销售才验证 
					if (isset($inventory[$row['invId']][$row['locationId']])) {
                        // str_alert(-1,$row['invName'].'库存不足！'.$inventory[$row['invId']][$row['locationId']]);
					    
                        $inventory[$row['invId']][$row['locationId']] < $row['qty'] && str_alert(-1,$row['locationName'].$row['invName'].'商品库存不足！库存数量：'.$inventory[$row['invId']][$row['locationId']]); 
					} else {
