...
class Report extends CI_Controller {
	...
	public function deliverSummary() {
		...
		$endDate    = str_enhtml($this->input->get_post('endDate',TRUE));
		$where  = 'a.isDelete=0';
		$where .= $storageNo ? ' and c.locationNo in('.str_quote($storageNo).')' : ''; //原 d.locationNo
		$where .= $goodsNo ? ' and b.number in('.str_quote($goodsNo).')' : ''; 
		$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : '';
		$where .= $this->common_model->get_location_purview();
		...
		$json['status'] = 200;
		$json['msg']    = 'success'; 
		$json['data']['stoNames']  = array("期初量","调拨入库","普通采购","销售退回","盘盈量","其他入库","成本调整","入库合计","调拨出库","采购退回","普通销售","盘亏量","其他出库","出库合计","期末量");
