...
class Inventory extends CI_Controller {
	...
	public function index() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['wholesalePrice']     = $row['wholesalePrice'];
	...
	public function listBySelected() { 
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['salePrice']    = $row['salePrice'];
            $v[$arr]['wholesalePrice']    = $row['wholesalePrice'];//12.29
			$v[$arr]['skuClassId']   = 0;
			$v[$arr]['skuId']        = 0;
			$v[$arr]['skuName']      = 0;
			$v[$arr]['skuNumber']    = 0;
	...
	public function add(){
		...
		if ($data) {
			...
			$this->db->trans_begin();
			$info = array(
			    'guanlianId','barCode','baseUnitId','unitName','categoryId','categoryName','propertys',
	...
	public function update(){
		...
		if ($data) {
			...
			$this->db->trans_begin();
			$info = array(
			    'barCode','baseUnitId','costUnitId','costQuantity','costName','unitName','categoryId','categoryName','propertys',
				'discountRate1','discountRate2','highQty','locationId','pinYin',
				'locationName','lowQty','name','number','purPrice','warehouseWarning',
				'remark','salePrice','spec','vipPrice','wholesalePrice','warehousePropertys'
			);
