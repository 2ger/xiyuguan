...
class Data_model extends CI_Model{
	...
	public function get_order_info($where='',$type=2) {
	    $sql = 'select 
					...
					b.unitName as mainUnit,b.pinYin,b.purPrice,b.quantity,b.salePrice,b.wholesalePrice,b.unitId,
				...
				from '.$this->db->dbprefix('order_info').' as a 
					left join 
						(select id,name,number,spec,unitName,unitId,pinYin,purPrice,quantity,salePrice,wholesalePrice from '.$this->db->dbprefix('goods').' where isDelete=0) as b 
					on a.invId=b.id 
					left join 
	...
	public function get_unhx($where='',$type=2) {
	    $sql = 'select 
		            a.*,ifnull(b.nowCheck,0) as nowCheck,(a.amount-ifnull(b.nowCheck,0)) as notCheck
				from '.$this->db->dbprefix('invoice').' as a 
					left join 
	...
	//获取仓库列表
	public function get_storage($type=2) {
	    $sql = 'select 
            *
            from '.$this->db->dbprefix('storage');
            return $this->mysql_model->query($sql,$type);
	}	
	//获取分店仓库信息
	public function get_location($iid,$type=2) {
	    $sql = 'select 
		            c.id,a.transType
				from '.$this->db->dbprefix('invoice').' as a 
					left join 
					    '.$this->db->dbprefix('contact').' as b 
					on a.buId=b.id
					left join '.$this->db->dbprefix('storage').' as c 
					on b.number=c.locationNo
				where a.id = '.$iid;
		return $this->mysql_model->query($sql,$type);
	}	
...
//用于 订货单修改商品时 下拉库存仅显示总部的， 2为总部库存ID
        if($where ==""){
         $where2 = " and locationId =2";
        }
	    $sql = 'select 
